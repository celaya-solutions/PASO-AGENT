...
Security reports are handled by the PASO maintainers at Celaya Solutions Research. See [CONTRIBUTING.md](CONTRIBUTING.md) for the project contribution process.
...
Report vulnerabilities for the CLI, gateway, desktop apps, mobile apps, and bundled integrations through a private [GitHub Security Advisory](https://github.com/celaya-solutions/PASO-AGENT/security/advisories/new).

If GitHub private reporting is unavailable, email **[hello@celayasolutions.com](mailto:hello@celayasolutions.com)** with the subject `Private PASO security report`. Do not open a public issue or PR that discloses an unpatched vulnerability, exploit path, secret, or security-sensitive proof of concept.
...
PASO does not currently run a paid bug bounty program. Please still disclose responsibly so we can fix real issues quickly. The best way to help the project right now is to send high-signal reports and, when practical, focused PRs.
...
- The actual impact, including which PASO trust boundary is crossed.
- Any remediation advice or focused patch you can provide.

Reports without reproduction steps, demonstrated impact, and remediation advice are deprioritized. We receive a high volume of AI-generated scanner findings, so we prioritize vetted reports from researchers who can show how the issue crosses a PASO security boundary.
...
- A report whose only primitive is changing the process or child-process environment before running PASO or an executable PASO invokes.
...
- Scanner-only, dependency-only, or stale-path reports without a working repro and demonstrated PASO impact.
...
- Tested version details (PASO version and/or commit SHA).
...
- For dependency CVE reports, evidence that the shipped dependency version is actually affected, plus a PoC that reproduces impact through PASO. Showing that PASO can reach a native parser is not enough by itself.
- Demonstrated impact tied to PASO's documented trust boundaries.
- For exposed-secret reports: proof the credential is PASO-owned (or grants access to PASO-operated infrastructure/services).
...
- Reports that depend on attacker-controlled environment variables changing executable behavior, including variables that redirect lookup paths, preload code, select wrappers/interpreters, alter package-manager or runtime hooks, or make one executable call another executable. Control of the process or child-process environment is trusted host/operator control in PASO's model; these reports need a separate PASO boundary bypass that lets untrusted input set or mutate that environment.
...
PASO does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- PASO can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need PASO, use one VPS (or host/OS user boundary) per user.
...
- This is expected in PASO's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

### Trusted Plugins

Plugins/extensions are part of PASO's trusted computing base for a gateway.
...
- Using PASO in ways that the docs recommend not to
- Test-only code and maintainer harnesses, including QA Lab, QE Lab, E2E fixtures, benchmark rigs, smoke-test containers, and local debugging proxies, unless the report demonstrates that the same vulnerable behavior is reachable from shipped PASO production code or a published package artifact intended for users.
...
- Exposed secrets that are third-party/user-controlled credentials (not PASO-owned and not granting access to PASO-operated infrastructure/services) without demonstrated PASO impact
...
PASO security guidance assumes:

- The host where PASO runs is within a trusted OS/admin boundary.
- Anyone who can set or mutate the PASO process environment, launcher environment, or child-process environment is inside that trusted host/operator boundary.
...
PASO's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
PASO separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when PASO can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the PASO process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

### Temp Folder Boundary

PASO uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:
...
- Sandbox media validation allows absolute temp paths only under the PASO-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use PASO temp helpers (`resolvePreferredOpenClawTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
...
PASO's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.
...
- The retired `gateway.controlUi.dangerouslyDisableDeviceAuth` break-glass key is not a
  ...
  - PASO keeps deployment flexibility by design and does not hard-forbid non-local setups.
...
PASO requires **Node.js 22.22.3+, Node.js 24.15+, or Node.js 25.9+**. Node 24 is the recommended default runtime for new installs. These minimum versions include the upstream SQLite WAL-reset corruption fix; Node 23 is unsupported. The minimum supported Node 22 version also includes important security patches:
...
When running PASO in Docker:

1. The locally built PASO container image runs as a non-root user (`node`) for reduced attack surface
...
Build `openclaw:local` from this repository first. Images named
`openclaw/openclaw` are upstream framework artifacts, not PASO releases.

## Security Scanning

PASO uses several security and release-validation layers. No single scanner is treated as the boundary.

### Secret Detection

PASO runs the pre-commit `detect-private-key` hook in CI and keeps secret-resolution behavior covered by the dedicated secrets test surface.
