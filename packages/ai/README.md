...
PASO. The package supports isolated runtime instances; importing it does not
...
`@openclaw/ai/validation`. No second PASO runtime package is required.

Provider ids, credentials, model catalogs, retries, and failover remain
application concerns. PASO supplies those policies around this package.
...
`runtime`, and `shared` subpaths exist for the PASO application itself.
They carry no semver guarantee and can change or disappear in any release; do
not depend on them outside PASO.
