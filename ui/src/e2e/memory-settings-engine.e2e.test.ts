  ...
  it("keeps the default engine first and drains Off before selecting it", async () => {
    await suite.withPage(
      ...
      async ({ page }) => {
        ...
        await engineGroup.getByRole("radio", { name: "PASO Memory", exact: true }).click();
    ...
    await suite.withPage(
      ...
      async ({ page }) => {
        ...
        await expect
          ...
          .toEqual(["PASO Memory (Unavailable)", "Memory LanceDB", "Off"]);
        await expect
          .poll(() =>
            engineGroup
              .getByRole("radio", { name: "PASO Memory (Unavailable)", exact: true })
