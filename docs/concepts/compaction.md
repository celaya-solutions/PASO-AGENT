---
summary: "How PASO summarizes long conversations to stay within model limits"
read_when:
...
Every model has a context window: the maximum number of tokens it can process. When a conversation approaches that limit, PASO **compacts** older messages into a summary so the chat can continue.
...
PASO keeps assistant tool calls paired with their matching `toolResult` entries when it picks a compaction split point. If the point lands inside a tool block, PASO moves the boundary so the pair stays together and the current unsummarized tail is preserved.
...
With the built-in safeguard quality guard enabled, PASO applies the final
...
Auto-compaction is on by default. It runs when the session nears the context limit, or when the model returns a context-overflow error (in which case PASO compacts and retries).

Stopping a run also stops its overflow or timeout recovery. The built-in PASO runtime does not start further recovery hooks, maintenance, transcript truncation, or retries after cancellation. Cancellation is not rollback: a compaction that already completed remains in the transcript and is still counted, without sending a late reply. The context estimate follows the latest model or compaction observation; billing totals remain separate.

Normal replies check session usage before the next turn. Successful direct commands using the built-in PASO runtime, including `openclaw agent --local`, run the same usage-based maintenance after recording the completed turn and protecting any pending reply. The following command then uses the compacted context. This works in safeguard mode even when memory flush is disabled; native runtimes retain their own compaction ownership.

If direct-command post-turn compaction fails, PASO logs a warning and returns the completed reply while the run and session are still current. Cancellation, restart, or a replaced session still stops that result from being returned.

Set `agents.defaults.compaction.enabled: false` to disable the embedded runtime's proactive threshold compaction and direct-command post-turn maintenance. PASO's preflight and overflow-recovery compaction paths remain available, as does manual `/compact`.
...
<Info>
Before compacting, PASO automatically reminds the agent to save important notes to [memory](/concepts/memory) files. This helps preserve durable context.
</Info>

<AccordionGroup>
  <Accordion title="Overflow error patterns PASO recognizes">
    PASO matches dozens of provider-specific overflow error strings (Anthropic, OpenAI, Bedrock, Gemini, Ollama, OpenRouter, and more). Common examples:
...
Client-side compaction in the built-in PASO runtime passes focus to both older-history and split-turn-prefix summaries. The host limits operator-provided focus to 800 Unicode code points and escapes it as prompt data before adding it to model requests.
...
When an embedded Responses provider returns a compacted window, PASO preserves the complete returned context alongside the checkpoint. Recent-turn history limits do not discard an eligible checkpoint, and the retained context still counts toward the model's prompt budget. The saved checkpoint is limited to 16 MiB; oversized or incompatible endpoint output uses the normal client-side compaction path instead of being truncated.

If an older version or transcript redaction removes the complete window needed for replay, PASO asks you to run `/compact`. That command rebuilds context from the saved conversation through client-side compaction. It does not guess the missing provider context or delete the transcript.
...
The built-in PASO runtime starts compaction with the active session model. Set `agents.defaults.compaction.model` to select a different summarization model. The override accepts a `provider/model-id` string or a bare alias configured under `agents.defaults.models`:
...
When unset, compaction starts with the active session model. If summarization fails with a model-fallback-eligible provider error, PASO retries that compaction attempt through the session's existing model fallback chain. The fallback choice is temporary and is not written back to session state. An explicit `agents.defaults.compaction.model` override remains exact and does not inherit the session fallback chain.
...
When `agents.defaults.compaction.maxActiveTranscriptBytes` is set, PASO
...
A context engine may return an explicit compacted successor session identity within the same agent, session key, and store. PASO publishes the accepted successor before maintenance, hooks, or retries use it, while retaining the current writer's ownership. Cancelling afterward does not roll that completed transition back. The built-in SQLite compactor keeps the current session identity and does not create a second runtime transcript.
...
PASO no longer writes separate `.checkpoint.*.jsonl` copies for new
...
Before compaction, PASO can run a **silent memory flush** turn to store durable notes to disk. Set `agents.defaults.compaction.memoryFlush.model` when this housekeeping turn should use a local model instead of the active conversation model:
...
Memory flush is optional maintenance: a failure, including exhausted retries, does not reset the session or discard conversation history. If compaction is unnecessary or succeeds, PASO continues the reply; with `notifyUser` enabled, exhausted flush retries also produce a degraded notice. If required compaction fails, PASO reports that failure and keeps the conversation intact instead of starting over automatically.
...
Plugins can register a custom compaction provider via `registerCompactionProvider()` on the plugin API. When a provider is registered and configured, PASO delegates summarization to it instead of the built-in LLM pipeline.
...
Setting a `provider` automatically forces `mode: "safeguard"`. Providers receive the same compaction instructions and identifier-preservation policy as the built-in path, and PASO still preserves recent-turn and split-turn suffix context after provider output.
...
<Note>
If the provider fails or returns an empty result, PASO falls back through the built-in safeguard summarizer and its configured quality checks. Provider-local timeouts do not bypass those checks; cancellation of the compaction request is still respected.
</Note>
