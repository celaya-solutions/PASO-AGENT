---
summary: "How PASO remembers things across sessions"
...
PASO remembers things by writing plain Markdown files in your agent's
...
If `MEMORY.md` grows past the bootstrap file budget, PASO keeps the file on
...
instead open **Settings → Ask PASO** and say `import memory`; this narrower
chat wizard requires completed onboarding, copies only new detected memory, and
reports per-source failures or possible partial copies. PASO copies only
Markdown memory:
...
PASO approval settings, sandboxing, and scheduled tasks for hard
...
<Info>
PASO uses OpenAI embeddings by default. Set
...
PASO runs a silent turn that reminds the agent to save important context
