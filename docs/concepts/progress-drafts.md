...
read_when:
  ...
  - Explaining how PASO updates one channel message while work is in progress
...
`channels.<channel>.streaming.mode: "progress"` and PASO creates the
...
that; otherwise PASO sends the final answer through normal delivery and
...
PASO renders only `progress.text` in the channel progress UI. The normal
...
PASO uses the same formatter for progress drafts and `/verbose`:
...
the draft is edited, and PASO truncates long lines so repeated draft edits
...
With `toolProgress: false`, PASO still suppresses the older standalone
...
When the final answer is ready, PASO tries to keep the chat clean:
...
- If the channel uses native progress streaming, PASO finalizes that
...
- Otherwise (media, an approval prompt, an explicit reply target, too many
  chunks, or a failed edit/send) PASO sends the final answer through the
...
Check `streaming.progress.toolProgress`. If it is `false`, PASO keeps the
