...
- `/usage cost` in chats: local cost summary aggregated from PASO session logs.
...
- Control UI: **Usage** shows provider plan and billing cards above PASO's session-derived token and estimated-cost analysis. Anthropic and OpenAI Admin API credentials add provider-reported today, 7-day, and 30-day spend, daily trends, token totals, top models, and cost categories.
...
- OpenAI ChatGPT/Codex OAuth continues to show plan, quota windows, and credit balance. Set `OPENAI_ADMIN_KEY` to show organization cost and completions-usage history instead; optionally set `OPENAI_PROJECT_ID` to scope it to one project. PASO never sends inference credentials from `OPENAI_API_KEY`, provider config, or auth profiles to organization APIs because those keys may belong to custom endpoints.

Admin credentials take precedence because they provide actual organization billing. PASO does not combine these provider-reported totals with its local session estimates; the two sections intentionally answer different questions.
...
Usage is hidden when no usable provider usage auth can be resolved. PASO
...
- **MiniMax**: API key or MiniMax OAuth auth profile. PASO treats
  ...
  quota, so PASO inverts them before display; count-based fields win when
  present.
  - Window labels come from provider hours/minutes fields when present, then
    fall back to the `start_time` / `end_time` span.
  - If the coding-plan endpoint returns `model_remains`, PASO prefers the
...
- **OpenAI (Codex/ChatGPT plan)**: OAuth tokens in auth profiles (`ChatGPT-Account-Id`
  ...
  credits; PASO does not label them as dollars. `OPENAI_ADMIN_KEY` adds
