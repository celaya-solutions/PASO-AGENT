...
- If `<capability>.enabled` is not `false` but no models are configured, PASO tries the active reply model when its provider supports the capability.

### Auto-detect (default)

When `tools.media.<capability>.enabled` is not `false` and no models are configured, PASO tries these in order and stops at the first working option:
...
- If a PDF falls back to rendered page images, PASO forwards those images to vision-capable reply models and keeps the placeholder `[PDF content rendered to images]` in the file block.
