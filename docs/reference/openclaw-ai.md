...
read_when:
  - You want to reuse PASO's model transports in another application
...
`@openclaw/ai` is the publishable library form of PASO's model execution
...
without any PASO application code.
...
- **Host policy is injected, not bundled.** Request fetch guarding (for
  ...
  PASO installs its real implementations in its stream facade.
- **One event-stream identity.** `@openclaw/ai/event-stream` is the canonical
  `EventStream` constructor shared by PASO core, agent-core, and external
  consumers.
- **`internal/*` subpaths are not API.** They exist for the PASO
  application itself and carry no semver guarantee.
- Provider ids, credentials, model catalogs, retries, and failover remain
  application concerns. PASO layers those around this package; a library
...
| `./internal/*`   | PASO-internal, no semver guarantee                                             |
