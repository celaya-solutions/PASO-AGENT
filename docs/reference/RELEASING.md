...
PASO preserves four compatibility update-channel names from the OpenClaw framework. Source
builds are the supported default for this fork. The npm and signed-app publication lanes below
are inherited operator procedures and remain disabled until Celaya-owned registry, repository,
and signing configuration is explicitly enabled.
...
`PASO Release Publish`. Its Docker-only extended-stable path rechecks the
...
11. For stable, continue only after the vetted beta or release candidate has the required validation evidence. Stable npm publish also goes through the compatibility workflow `PASO Release Publish`, reusing the successful preflight artifact via `preflight_run_id`. Stable macOS release readiness also requires the packaged `.zip`, `.dmg`, `.dSYM.zip`, and updated `appcast.xml` on `main`; the macOS publish workflow publishes the signed appcast to public `main` automatically after release assets verify, or opens/updates an appcast PR if branch protection blocks the direct push. Stable Windows readiness requires the signed `OpenClawCompanion-Setup-x64.exe`, `OpenClawCompanion-Setup-arm64.exe`, and `OpenClawCompanion-SHA256SUMS.txt` compatibility assets on the PASO GitHub release. Before using this lane, configure `PASO_WINDOWS_NODE_REPOSITORY`, `PASO_WINDOWS_SIGNER_SUBJECT`, and `PASO_WINDOWS_SIGNER_THUMBPRINT` with Celaya-owned release infrastructure. Pass the exact signed release tag from that configured repository as `windows_node_tag` and its candidate-approved installer digest map as `windows_node_installer_digests`; the publish workflow otherwise fails closed.
...
- `PASO NPM Release` preflight generates dependency release evidence before it packs the npm tarball. The npm advisory vulnerability gate is release-blocking. The transitive manifest risk, dependency ownership/install surface, and dependency change reports are release evidence only. The dependency change report compares the release candidate with the previous reachable release tag. The preflight uploads dependency evidence as `openclaw-release-dependency-evidence-<tag>` and also embeds it under `dependency-evidence/` inside the prepared npm preflight artifact. The real publish path reuses that preflight artifact, then attaches the same evidence to the GitHub release as `openclaw-<version>-dependency-evidence.zip`.
- Run `PASO Release Publish` for the mutating publish sequence after the tag exists. Dispatch regular beta and stable publishes from trusted `main`; the release tag still selects the exact target commit and may point into `release/YYYY.M.PATCH`. Tideclaw alpha publishes remain on their matching alpha branch. Pass the successful OpenClaw npm `preflight_run_id`, successful `full_release_validation_run_id`, and exact `full_release_validation_run_attempt`, and keep the default plugin publish scope `all-publishable` unless you are deliberately running a focused repair. The workflow serializes plugin npm publish, plugin ClawHub publish, and OpenClaw npm publish so the core package is not published before its externalized plugins; Windows promotion and Android dispatch run concurrently with the core npm publish. Android approval, build, and publication are monitored separately and do not hold core publication; the child can attach its verified assets after the GitHub release becomes public. Publish reruns are resumable: an already-published core npm version skips the core dispatch after the workflow proves the registry tarball matches the tag's preflight artifact, and Windows/Android promotion is skipped when the release already carries the verified asset contract, so a retry only redoes the failed stages. Focused plugin-only repairs require `plugin_publish_scope=selected` and a nonempty plugin list. Plugin-only `all-publishable` runs require complete immutable preflight and Full Release Validation evidence; partial evidence is rejected.
- Stable `PASO Release Publish` requires an exact `windows_node_tag` after the matching non-prerelease release exists in `PASO_WINDOWS_NODE_REPOSITORY`, plus the candidate-approved `windows_node_installer_digests` map. The source repository must be owned by `celaya-solutions`. Before dispatching any publish child, the parent verifies that source release is published, non-prerelease, contains the required x64/ARM64 installers, and still matches that approved map. It also requires a Celaya Solutions signer subject and an exact configured certificate thumbprint. The child workflow downloads the signed PASO Windows installers from that exact tag, matches them against the pinned digests, verifies their Authenticode subject and thumbprint on a Windows runner, writes a SHA-256 manifest, and uploads the installers plus manifest onto the PASO GitHub release. Missing configuration, an upstream signer, or any identity mismatch fails closed. The `OpenClawCompanion-*` filenames remain compatibility artifact names.

  Manually dispatch `Windows Node Release` only for recovery, and always pass an exact tag, never `latest`, plus the explicit `expected_installer_digests` JSON map from the approved source release. Website download links should target exact PASO release asset URLs for the current stable release, or `releases/latest/download/...` only after verifying GitHub's latest redirect points at that same release; do not link only to the source repository release page.

- Release checks now run in a separate manual workflow: `PASO Release Checks`. It always runs the QA Lab mock parity lane. The Matrix catalog and Telegram QA-live lanes run for stable/full validation, soak-enabled validation, or an explicit `qa`/`qa-live` rerun group. Bounded beta-publish `all` without soak defers those live lanes to postpublish-confidence. The live lanes use the `qa-live-shared` environment; Telegram also uses Convex CI credential leases.
- Cross-OS install and upgrade runtime validation is part of public `PASO Release Checks` and `Full Release Validation`, which call the reusable workflow `.github/workflows/openclaw-cross-os-release-checks-reusable.yml` directly. This split is intentional: keep the real npm release path short, deterministic, and artifact-focused, while slower live checks stay in their own lane so they do not stall or block publish.
...
- Maintainer release automation uses preflight-then-promote:
  ...
  - Token-based npm dist-tag mutation is not configured for PASO. Do not invoke inherited upstream automation. Keep promotion disabled until Celaya Solutions owns and documents the repository, workflow, and `NPM_TOKEN` route.
...
The Docker box lives in `PASO Release Checks` through `openclaw-live-and-e2e-checks-reusable.yml`, plus the release-mode `install-smoke` workflow. It validates the release candidate through packaged Docker environments instead of only source-level tests.
...
The QA Lab box is also part of `PASO Release Checks`. It is the agentic behavior and channel-level release gate, separate from Vitest and Docker package mechanics.
...
`PASO Release Publish` is the normal mutating entrypoint. The monthly
...
6. Dispatch `PASO NPM Release` with the release tag, npm dist-tag, and saved `preflight_run_id` after verifying the saved `full_release_validation_run_id` and exact run attempt.
...
release tag plus the parent `PASO Release Publish` run id, attempt, and
...
`PASO NPM Release` accepts these operator-controlled inputs:
...
- `release_publish_run_id`: approved `PASO Release Publish` run id; required when this workflow is dispatched by that parent (bot-actor real-publish calls)
...
`PASO Release Publish` accepts these operator-controlled inputs:

- `tag`: required release tag; must already exist
- `preflight_run_id`: successful `PASO NPM Release` preflight run id; required when `publish_openclaw_npm=true` or `plugin_publish_scope=all-publishable`
...
- `windows_node_tag`: exact non-prerelease tag from the Celaya-owned repository configured by `PASO_WINDOWS_NODE_REPOSITORY`; required for stable PASO publish
- `windows_node_installer_digests`: candidate-approved compact JSON map of the current Windows installer names to their pinned `sha256:` digests; required for stable PASO publish
...
`PASO Release Checks` accepts these operator-controlled inputs:
...
- For `PASO NPM Release`, full commit SHA input is allowed only when `preflight_only=true`
- `PASO Release Checks` and `Full Release Validation` are always validation-only
...
1. Run `PASO NPM Release` with `preflight_only=true`. Before a tag exists, you may use the current full workflow-branch commit SHA for a validation-only dry run of the preflight workflow.
...
4. After the PASO Windows repository, signer subject, and signer thumbprint variables are configured, select the exact non-prerelease release tag whose Celaya-signed x64 and ARM64 installers should ship. Save it as `windows_node_tag`, and save their validated digest map as `windows_node_installer_digests`. The release-candidate helper records both and includes them in its generated publish command.
5. Save the successful `preflight_run_id`, `full_release_validation_run_id`, and exact `full_release_validation_run_attempt`.
6. Run `PASO Release Publish` from trusted `main` with the same `tag`, the same `npm_dist_tag`, the selected `windows_node_tag`, its saved `windows_node_installer_digests`, the saved `preflight_run_id`, `full_release_validation_run_id`, and `full_release_validation_run_attempt`. It publishes externalized plugins to npm and ClawHub before promoting the OpenClaw npm package.
7. If the release landed on `beta`, stop. PASO has no configured dist-tag mutation lane. Provision and review a Celaya-owned repository, workflow, and token route before promoting it to `latest`.
8. If the release intentionally published directly to `latest`, leave `beta` unchanged until that same Celaya-owned route exists and has been reviewed.

Dist-tag mutation still requires `NPM_TOKEN`, while this source repository keeps OIDC-only publish. PASO therefore fails closed instead of delegating that mutation to an upstream-owned release ledger.
...
- [`.github/workflows/full-release-validation.yml`](https://github.com/celaya-solutions/PASO-AGENT/blob/main/.github/workflows/full-release-validation.yml)
- [`.github/workflows/package-acceptance.yml`](https://github.com/celaya-solutions/PASO-AGENT/blob/main/.github/workflows/package-acceptance.yml)
- [`.github/workflows/openclaw-npm-release.yml`](https://github.com/celaya-solutions/PASO-AGENT/blob/main/.github/workflows/openclaw-npm-release.yml)
- [`.github/workflows/openclaw-release-checks.yml`](https://github.com/celaya-solutions/PASO-AGENT/blob/main/.github/workflows/openclaw-release-checks.yml)
- [`.github/workflows/openclaw-cross-os-release-checks-reusable.yml`](https://github.com/celaya-solutions/PASO-AGENT/blob/main/.github/workflows/openclaw-cross-os-release-checks-reusable.yml)
- [`.github/workflows/docker-release.yml`](https://github.com/celaya-solutions/PASO-AGENT/blob/main/.github/workflows/docker-release.yml)
- [`scripts/resolve-openclaw-package-candidate.mts`](https://github.com/celaya-solutions/PASO-AGENT/blob/main/scripts/resolve-openclaw-package-candidate.mts)
- [`scripts/openclaw-npm-release-check.ts`](https://github.com/celaya-solutions/PASO-AGENT/blob/main/scripts/openclaw-npm-release-check.ts)
- [`scripts/package-mac-dist.sh`](https://github.com/celaya-solutions/PASO-AGENT/blob/main/scripts/package-mac-dist.sh)
- [`scripts/make_appcast.sh`](https://github.com/celaya-solutions/PASO-AGENT/blob/main/scripts/make_appcast.sh)

The PASO fork does not rely on the inaccessible private upstream maintainer
runbook. This public file and the fork-owned workflows above document the PASO
release path; upstream release infrastructure is a separate compatibility
system.
