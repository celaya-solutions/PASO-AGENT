...
read_when:
  ...
  - You need the numbers behind the PASO performance and dependency blog post
...
This page captures the evidence behind the May 2026 PASO performance,
...
- **Release performance sweep:** GitHub Releases from `v2026.5.28` back through
  stable `v2026.4.23`, using the `PASO Performance` workflow,
...
<CardGroup cols={2}>
  ...
  <Card title="Latest stable install" icon="hard-drive">
    **361.7MiB fresh install**
    ...
    Cuts the nested PASO dependency tree sharply from the `2026.5.22`
...
| Point              | Installed deps | Fresh install | PASO package | Nested `openclaw/node_modules` | Root shrinkwrap | Canvas install behavior                   |
| ------------------ | -------------: | ------------: | -----------: | -----------------------------: | --------------- | ----------------------------------------- |
...
`2026.5.20` shipped with no root shrinkwrap and no large nested PASO
...
that made npm materialize a large nested PASO dependency tree and all 12
