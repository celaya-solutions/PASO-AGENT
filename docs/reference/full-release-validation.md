...
workflow once. Plugin Prerelease and PASO Release Checks each dispatch an
...
| Product performance     | **Job:** `Run product performance evidence`<br />**Child workflow:** `PASO Performance`<br />**Proves:** release-profile performance run (`profile=release`, `repeat=3`, `fail_on_regression=true`, `publish_reports=false`) against the target SHA. Kova output stays in workflow artifacts and the child must prove its report publisher was skipped. Required (blocking) only for `rerun_group=all` or `rerun_group=performance`; not required for narrower rerun groups.<br />**Rerun:** `rerun_group=performance`.                                                          |
...
`PASO Performance` permits report publication only for scheduled runs or a
...
`PASO Release Checks` is the largest child workflow. It resolves the target
...
| Cross-OS                 | **Job:** `cross_os_release_checks`<br />**Backing workflow:** `PASO Cross-OS Release Checks (Reusable)`<br />**Tests:** fresh and upgrade lanes on Linux, Windows, and macOS for the selected provider and mode, using the candidate tarball plus a baseline package.<br />**Rerun:** `rerun_group=cross-os`.                                                                                                                                                                                                                                                                     |
| Repo and live E2E        | **Job:** `Run repo/live E2E validation`<br />**Backing workflow:** `PASO Live And E2E Checks (Reusable)`<br />**Tests:** repository E2E, live cache, OpenAI websocket streaming, native live provider and plugin shards, and Docker-backed live model/backend/gateway harnesses selected by `release_profile`.<br />**Runs:** `run_release_soak=true`, `release_profile=full`, or focused `rerun_group=live-e2e`.<br />**Rerun:** `rerun_group=live-e2e`, optionally with `live_suite_filter`.                                                                                    |
| Docker release path      | **Job:** `Run Docker release-path validation`<br />**Backing workflow:** `PASO Live And E2E Checks (Reusable)`<br />**Tests:** release-path Docker chunks against the shared package artifact.<br />**Runs:** `run_release_soak=true`, `release_profile=full`, or focused `rerun_group=live-e2e`.<br />**Rerun:** `rerun_group=live-e2e`.                                                                                                                                                                                                                                         |
...
| QA live Telegram         | **Job:** `Run QA Lab live Telegram lane`<br />**Backing workflow:** trusted `PASO Release Telegram QA` dispatch<br />**Tests:** live Telegram QA with Convex CI credential leases.<br />**Rerun:** `rerun_group=qa-live`; direct manual child dispatch may aggregate with `qa`.                                                                                                                                                                                                                                                                                                   |
...
The umbrella/controller also rejects `qa`; direct `PASO Release Checks`
...
manual `PASO Release Checks` dispatch may use `qa` to aggregate both.
...
- `release-package-under-test` from `PASO Release Checks`
