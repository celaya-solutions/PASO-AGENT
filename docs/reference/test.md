...
PASO wrapper and workflow inputs. Routine task-needed Crabbox/Testbox use
...
Run trusted PASO remote proof through the wrapper from the repository root:
...
when host caches cannot cross filesystems. The PASO broker login endpoint
...
the always-bundled PASO packages. Each generation carries all seven fs-safe
...
| `pnpm test:force`                                 | Frees the configured PASO gateway port (default `18789`), then runs the full suite with an isolated gateway port so server tests do not collide with a running instance.                                                                                                                                                                              |
...
| `pnpm test:docker:openwebui`                                                                 | Dockerized PASO + Open WebUI: sign in, check `/api/models`, run a real proxied chat through `/api/chat/completions`. Requires a usable live model key and pulls an external image; not expected to be CI-stable like the unit/e2e suites.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `pnpm test:docker:package-install`           | Packed PASO npm artifact installation into a clean global prefix, then CLI version and help startup from the installed package.                                                                                                                                    |
