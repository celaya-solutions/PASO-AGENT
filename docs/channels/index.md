---
summary: "Messaging platforms PASO can connect to"
read_when:
  - You want to choose a chat channel for PASO
  - You need a quick overview of supported messaging platforms
title: "Chat channels"
---

PASO can talk to you on any chat app you already use. Each channel connects via the Gateway.
...
"External plugin" channels are maintained outside the PASO repository.
...
- [A2A](/channels/a2a) - Connect external agents to PASO through the A2A 1.0 JSON-RPC protocol (bundled plugin).
- [Buzz](/channels/buzz) - Connect PASO agents to Buzz rooms (official plugin).
...
- [iMessage](/channels/imessage) - Native iMessage support via imsg (JSON-RPC over stdio), with private API actions for replies, tapbacks, effects, polls, attachments, and group management. Preferred for new PASO iMessage setups when host requirements fit (official plugin).
...
- [Mattermost](/channels/mattermost) - Mattermost bot setup and PASO config (official plugin).
...
- [Reef](/channels/reef) - Reef channel setup: guarded, end-to-end-encrypted messaging between PASO agents of different people (bundled plugin).
...
- [Synology Chat](/channels/synology-chat) - Synology Chat webhook setup and PASO config (official plugin).
...
- Channels can run simultaneously; configure multiple and PASO will route per chat.
