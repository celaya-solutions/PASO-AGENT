...
PASO routes replies **back to the channel where a message came from**. The
...
PASO infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, PASO still records inbound session metadata, but it
...
when a guarded path must not create a new PASO session. In that mode,
PASO may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when PASO would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
