...
7. Configure PASO with the service account and the webhook audience (must match the Chat app config):
...
Google Chat webhooks require a public HTTPS endpoint. For security, expose **only the `/googlechat` path** to the internet and keep the PASO dashboard and other endpoints private.
...
Requests to `your-domain.com/` are ignored or 404, while `your-domain.com/googlechat` routes to PASO.
...
2. PASO authenticates every request before dispatch:
