...
Matrix reply streaming is opt-in. `streaming.mode` controls how PASO delivers the in-flight assistant reply; `streaming.block.enabled` controls whether each completed block is kept as its own Matrix message.
...
- If a preview grows past Matrix's per-event size limit, PASO stops preview streaming and falls back to final-only delivery.
- Media replies always send attachments normally. If a visible preview cannot be reused safely, PASO keeps it until the complete replacement is confirmed and then redacts it. If replacement delivery fails, is partial, or produces no visible event, the preview remains visible.
...
- In encrypted rooms, PASO decrypts the attachment through the existing Matrix media path before transcription.
...
Matrix native approval prompts are normal `m.room.message` events with PASO-specific content under the `com.openclaw.approval` key. Stock clients still render the text body; PASO-aware clients can read the structured approval id, kind, state, decisions, and exec/plugin details.

When a prompt is too long for one Matrix event, PASO chunks the visible text and attaches `com.openclaw.approval` to the first chunk only. Allow/deny reactions bind to that first event, so long prompts keep the same approval target as single-event prompts.
...
By default, Matrix messages from other configured PASO Matrix accounts are ignored. Use `allowBots` to intentionally allow inter-agent traffic:
...
- PASO still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix has no native bot flag; PASO treats "bot-authored" as "sent by another configured Matrix account on this PASO gateway".
...
If the homeserver requires UIA to upload cross-signing keys, PASO tries no-auth first, then `m.login.dummy`, then `m.login.password` (requires `channels.matrix.password`).
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    ...
    Startup also runs a conservative crypto bootstrap pass reusing the current secret storage and cross-signing identity. If bootstrap state is broken, PASO attempts a guarded repair even without `channels.matrix.password`; if the homeserver requires password UIA, startup logs a warning and stays non-fatal. Already-owner-signed devices are preserved.
  ...
  <Accordion title="Verification notices">
    ...
    Incoming requests from another Matrix client are tracked and auto-accepted. For self-verification, PASO starts the SAS flow automatically and confirms its own side once emoji verification is available - you still need to compare and confirm "They match" in your Matrix client.
  ...
  <Accordion title="Deleted or invalid Matrix device">
    If `verify status` says the current device is no longer listed on the homeserver, create a new PASO Matrix device. For password login:
...
openclaw matrix account add \
  ...
  --password '<password>' \
  --device-name PASO-Gateway
```

    For token auth, create a fresh access token in your Matrix client or admin UI, then update PASO:
...
```
  ...
  <Accordion title="Device hygiene">
    Old PASO-managed devices can accumulate. List and prune:
...
```
  ...
  <Accordion title="Crypto store">
    ...
    Encrypted runtime state lives under `~/.openclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/` and includes the sync store, crypto store, recovery key, IDB snapshot, thread bindings, and startup verification state. When the token changes but the account identity stays the same, PASO reuses the best existing root so prior state remains visible.

    A single older token-hash root can be a normal token-rotation continuity path. If PASO logs `matrix: multiple populated token-hash storage roots detected`, inspect the account directory and archive stale sibling roots only after confirming the selected active root is healthy. Prefer moving stale roots into an `_archive/` directory over deleting them immediately.
...
```bash
openclaw matrix profile set --name "PASO Assistant"
...
`dm.sessionScope` decides how Matrix DM rooms map to PASO sessions:
...
- DM user-target reuse only kicks in when current session metadata proves the same DM peer on the same Matrix account; otherwise PASO falls back to normal user-scoped routing.
...
When PASO detects a Matrix DM room colliding with another DM room on the same shared session, it posts a one-time `m.notice` suggesting `dm.sessionScope: "per-room"` to isolate the rooms. The notice only appears when thread bindings are enabled.
...
`--bind here` does not create a child Matrix thread. `threadBindings.spawnSessions` gates `/acp spawn --thread auto|here`, where PASO needs to create or bind a child thread.
...
- Room history is pending-only: PASO buffers room messages that did not trigger a reply yet, then snapshots that window when a mention or other trigger arrives.
...
If an unapproved Matrix user keeps messaging before approval, PASO reuses the same pending pairing code and may send a reminder reply after a short cooldown instead of minting a new code.

See [Pairing](/channels/pairing) for the shared DM pairing flow and storage layout.

## Direct room repair

If direct-message state drifts, PASO can end up with stale `m.direct` mappings pointing at old solo rooms instead of the live DM. Inspect the current mapping for a peer:
...
Slash commands (`/new`, `/reset`, `/model`, `/agents`, `/session`, `/acp`, `/approve`, etc.) work directly in DMs. In rooms, PASO also recognizes commands prefixed with the bot's own Matrix mention, so `@bot:server /new` triggers the command path without a custom mention regex - this keeps the bot responsive to the room-style `@mention /command` posts that Element and similar clients emit when a user tab-completes the bot before typing the command.
...
- If you have multiple accounts and one is literally named `default`, PASO uses it implicitly even when `defaultAccount` is unset.
...
- When PASO promotes a single-account config to multi-account during repair or setup, it preserves the existing named account if one exists or `defaultAccount` already points at one. Only Matrix auth/bootstrap keys move into the promoted account; shared delivery-policy keys stay at the top level.

See [Configuration reference](/gateway/config-channels#multi-account-all-channels) for the shared multi-account pattern.

## Private/LAN homeservers

By default, PASO blocks private/internal Matrix homeservers for SSRF protection unless you opt in per account.
...
Named accounts can override the top-level default with `channels.matrix.accounts.<id>.proxy`. PASO uses the same proxy setting for runtime Matrix traffic and account status probes.

## Target resolution

Matrix accepts these target forms anywhere PASO asks for a room or user target:
...
Matrix room IDs are case-sensitive. Use the exact room ID casing from Matrix when configuring explicit delivery targets, cron jobs, bindings, or allowlists. PASO keeps internal session keys canonical for storage, so those lowercase keys are not a reliable source for Matrix delivery IDs.
