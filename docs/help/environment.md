---
summary: "Where PASO loads environment variables and the precedence order"
read_when:
...
PASO pulls environment variables from multiple sources. The normal rule is **never override existing values**. For a PASO-installed systemd service, the global `.env` may replace only service values that PASO recorded as managed; operator-owned service values still take precedence.
Workspace `.env` files are a lower-trust source: PASO ignores provider credentials and protected runtime controls from workspace `.env` before applying precedence.
...
3. **Global `.env`** at `~/.openclaw/.env` (aka `$OPENCLAW_STATE_DIR/.env`; recommended for provider API keys; does not override except for recorded PASO-managed systemd service values).
4. **Config `env` block** in `~/.openclaw/openclaw.json` (applied only if missing).
5. **Optional login-shell import** (`env.shellEnv.enabled` or `OPENCLAW_LOAD_SHELL_ENV=1`), applied only for missing expected keys.

On fresh Ubuntu installs that use the default state dir, PASO also treats `~/.config/openclaw/gateway.env` as a compatibility fallback after the global `.env`. If both files exist and disagree, PASO keeps `~/.openclaw/.env` and prints a warning.
...
| Variable                 | Purpose                                                           |
| ------------------------ | ----------------------------------------------------------------- |
| `OPENCLAW_HOME`          | Override the home directory used for PASO path defaults.          |
...
Installed third-party plugins may declare additional credential variables in their plugin manifests; those variables are contracts of the plugin that declares them, not core PASO variables.
...
Do not keep provider API keys only in a workspace `.env`. PASO blocks a large set of provider credential and endpoint-redirect keys from workspace `.env` files, including every known provider auth env var (for example `GEMINI_API_KEY`, `GOOGLE_API_KEY`, `XAI_API_KEY`, `MISTRAL_API_KEY`, `GROQ_API_KEY`, `DEEPSEEK_API_KEY`, `PERPLEXITY_API_KEY`, `BRAVE_API_KEY`, `TAVILY_API_KEY`, `EXA_API_KEY`, `FIRECRAWL_API_KEY`), plus any key ending in `_API_HOST`, `_BASE_URL`, `_ENDPOINT`, or `_HOMESERVER`, and the entire `OPENCLAW_*`, `CLAWHUB_*`, `ANTHROPIC_API_KEY_*`, and `OPENAI_API_KEY_*` namespaces.
...
failures can affect PASO startup. Other shells use noninteractive login startup (`-l -c`).
...
PASO also injects context markers into spawned child processes:
...
- `COLORFGBG`: if your terminal exports it, PASO uses the background color hint to auto-pick the TUI palette.
...
PASO supports two env-driven patterns:
...
| Variable                 | Purpose                                                                                                                                                                                                                         |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENCLAW_HOME`          | Override the home directory used for internal PASO path defaults (`~/.openclaw/`, agent dirs, sessions, credentials, installer onboarding, and the default dev checkout). Useful when running PASO as a dedicated service user. |
...
Set `OPENCLAW_OFFLINE=1` to prevent PASO from downloading its pinned `fd`
and `ripgrep` helper binaries. Existing helpers under the PASO tools
...
When set, `OPENCLAW_HOME` replaces the system home directory (`$HOME` / `os.homedir()`) for internal PASO path defaults. This includes the default state directory, config path, agent directories, credentials, installer onboarding workspace, and the default dev checkout used by `openclaw update --channel dev`.
...
On Linux, PASO automatically detects nvm and applies the fix in the actual startup environment:
...
Export the variable before starting PASO:
...
PASO only reads `OPENCLAW_*` environment variables. The legacy
`CLAWDBOT_*` and `MOLTBOT_*` prefixes from earlier releases are silently
ignored.

If any are still set on the Gateway process at startup, PASO emits a
