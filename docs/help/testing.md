...
PASO has three Vitest suites (unit/integration, e2e, live) plus Docker
...
- Runtime performance reports: dispatch `PASO Performance` with
...
- Docker live model sweep: `pnpm test:docker:live-models`
  ...
  - CI coverage: daily `PASO Scheduled Live And E2E Checks` and manual
    `PASO Release Checks` both call the reusable live/E2E workflow with
    `include_live_suites: true`, which includes Docker live model matrix jobs
    sharded by provider.
  - For focused CI reruns, dispatch `PASO Live And E2E Checks (Reusable)`
...
- Codex on-demand install smoke: `pnpm test:docker:codex-on-demand`
  - Installs the packaged PASO tarball in Docker, runs OpenAI API-key
...
- Codex npm-plugin live package smoke: `pnpm test:docker:live-codex-npm-plugin`
  - Installs the candidate PASO package and exact Codex plugin into Docker,
...
- PASO rescue command smoke: `pnpm test:live:system-agent-rescue-channel`
  - Opt-in belt-and-suspenders check for the message-channel rescue command
    ...
    path.
- PASO first-run Docker smoke: `pnpm test:docker:system-agent-first-run`
  - Starts from an empty PASO state dir and first proves the packaged
    ...
    PASO agent/tool/approval proof. The same lane is exposed in QA Lab by
...
The direct `PASO Release Checks` child alone may use `rerun_group=qa` as a
...
- `pnpm test:docker:session-runtime-context`
  - Runs a deterministic built-app Docker smoke for embedded runtime context
    transcripts. Verifies hidden PASO runtime context persists as a
...
- `pnpm test:docker:npm-telegram-live`
  - Installs a PASO package candidate in Docker, runs installed-package
...
- `pnpm test:docker:plugins`
  - Packs and installs the current PASO build in Docker, starts the
...
- Compare native opus performance in the `libopus-wasm` benchmark repo, not
  in default PASO install/test loops. Do not set `@discordjs/opus` to
...
<AccordionGroup>
  ...
  <Accordion title="Fast local iteration">
    ...
    - The pre-commit hook formats and restages files. When private rules are
      ...
      See [Local commit hook setup](https://github.com/celaya-solutions/PASO-AGENT/blob/main/CONTRIBUTING.md#local-commit-hook).
...
- Docker/Bash E2E lanes that install the packed PASO tarball through `scripts/lib/openclaw-e2e-instance.sh` cap `npm install` at `OPENCLAW_E2E_NPM_INSTALL_TIMEOUT` (default `600s`; set `0` to disable the wrapper for debugging).
...
- Bun global install and runtime smoke: `bash scripts/e2e/bun-global-install-smoke.sh` packs the current tree, installs it with `bun install -g --trust` in an isolated home, verifies PASO's lifecycle scripts ran, and executes the installed package with Bun 1.4 or newer. It checks representative CLI state, bundled image providers, a mocked local agent turn, Gateway readiness and health, and a mocked agent turn through the Bun-hosted Gateway. Reuse a prebuilt tarball with `OPENCLAW_BUN_GLOBAL_SMOKE_PACKAGE_TGZ=/path/to/openclaw-*.tgz`, skip the host build with `OPENCLAW_BUN_GLOBAL_SMOKE_HOST_BUILD=0`, or copy `dist/` from a built Docker image with `OPENCLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE=openclaw-dockerfile-smoke:local`.
...
- PASO bundle MCP tools (real stdio MCP server + embedded PASO profile allow/deny smoke): `pnpm test:docker:agent-bundle-mcp-tools` (script: `scripts/e2e/agent-bundle-mcp-tools-docker.sh`)
...
- Plugin lifecycle matrix smoke: `pnpm test:docker:plugin-lifecycle-matrix` installs the packed PASO tarball in a bare container, installs an npm plugin, toggles enable/disable, upgrades and downgrades it through a local npm registry, deletes the installed code, then verifies uninstall still removes stale state while logging RSS/CPU metrics for each lifecycle phase.
...
PASO gateway container with the OpenAI-compatible HTTP endpoints enabled,
...
embedded PASO bundle MCP runtime, executes the tool, then verifies
