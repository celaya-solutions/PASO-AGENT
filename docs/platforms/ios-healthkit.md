...
PASO can request a read-only summary of the current calendar day from a
...
- An iPhone or iPad running the PASO iOS app where HealthKit reports health data as
  available.
...
- Readable Health data for any metrics you expect to see. An Apple Watch can
  contribute data to the Apple Health store, but the PASO watchOS app is
...
3. Read the disclosure, then choose which Health categories PASO may read
   in Apple's permission sheet.

The switch records your explicit PASO sharing choice. It does not claim
...
- PASO requests read access only. It cannot add or modify Health data.
- PASO reads HealthKit only when `health.summary` is invoked. There is no
...
- HealthKit deliberately does not reveal whether read access was denied. A
  ...
  data type. PASO cannot distinguish those cases.
...
PASO's access in Apple's Health settings, but do not treat an empty result
