...
read_when:
  - Looking for an overview of PASO's media capabilities
...
PASO generates images, videos, and music, understands inbound media
...
PASO until the shared realtime-voice contract can represent it.
</Note>
...
For async tools, PASO submits the request to the provider, returns a task
...
PASO wakes the agent with the generated media paths so it can tell the
...
PASO sends an idempotent direct fallback with only the missing media. Media
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    other native model types; PASO has no provider contract for those
  ...
  <Accordion title="xAI">
    ...
    not registered in PASO until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
