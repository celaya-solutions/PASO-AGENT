---
summary: "Export redacted trajectory bundles for debugging a PASO agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for a PASO session
...
Trajectory capture is PASO's per-session flight recorder. It records a
...
PASO writes the bundle under the workspace:
...
bundle; do not use allow-all. In group chats, PASO sends the approval
...
Trajectory capture is on by default for PASO agent runs.
...
| `metadata.json`       | PASO version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata         |
...
This disables runtime trajectory capture before starting PASO.
...
PASO flushes runtime trajectory rows during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`OPENCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting PASO:

```bash
export OPENCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when PASO logs an `openclaw-trajectory-flush` timeout and
...
Trajectory bundles are for support and debugging, not public posting. PASO
...
- confirm PASO was started without `OPENCLAW_TRAJECTORY=0`
