---
summary: "Run external coding harnesses (Claude Code, Cursor, Gemini CLI, explicit Codex ACP, PASO ACP, OpenCode) through the ACP backend"
read_when:
...
PASO run external coding harnesses (Claude Code, Cursor, Copilot, Droid,
PASO ACP, OpenCode, Gemini CLI, and other supported ACPX harnesses)
...
existing PASO channel conversations, use
...
| Run Claude Code, Gemini CLI, explicit Codex ACP, or another external harness _through_ PASO | This page                             | Chat-bound sessions, `/acp spawn`, `sessions_spawn({ runtime: "acp" })`, background tasks, runtime controls                                                                 |
| Expose a PASO Gateway session _as_ an ACP server for an editor or client                    | [`openclaw acp`](/cli/acp)            | Bridge mode: an IDE/client speaks ACP to PASO over stdio/WebSocket                                                                                                          |
| Reuse a local AI CLI as a text-only fallback model                                          | [CLI Backends](/gateway/cli-backends) | Not ACP: no PASO tools, no ACP controls, no harness runtime                                                                                                                 |
...
PASO only teaches agents about ACP spawning when ACP is **truly usable**:
...
<AccordionGroup>
  ...
  <Accordion title="Runtime prerequisites">
    ACP launches a real external harness process. PASO owns routing,
...
PASO plugin tools and built-in PASO tools are **not** exposed to ACP
...
| `openclaw`   | PASO Gateway bridge through `openclaw acp` | Lets an ACP-aware harness talk back to a PASO Gateway session.                      |
...
Custom acpx agent aliases can be configured in acpx itself, but PASO
...
<AccordionGroup>
  <Accordion title="Lifecycle details">
    - Spawn creates or resumes an ACP runtime session, records ACP metadata in the PASO session store, and may create a background task when the run is parent-owned.
    ...
    - `close` ends the ACP session from PASO's point of view and removes the binding. A harness may still keep its own upstream history if it supports resume.
    - The acpx plugin cleans up PASO-owned wrapper and adapter process trees after `close`, and reaps stale PASO-owned ACPX orphans during Gateway startup.
  ...
  <Accordion title="Native Codex routing rules">
    ...
    PASO dynamic tools still execute through PASO, while Codex-native
    tools such as shell/apply-patch execute inside Codex. For Codex-native
    tool events, PASO injects a per-turn native hook relay so plugin hooks
    can block `before_tool_call`, observe `after_tool_call`, and route Codex
    `PermissionRequest` events through PASO approvals. Codex `Stop` hooks
    are relayed to PASO `before_agent_finalize`, where plugins can request
  ...
  <Accordion title="ACP-routing natural-language triggers">
    ...
    PASO picks `runtime: "acp"`, resolves the harness `agentId`, binds to
    ...
    `gemini`, or `opencode`. Do not pass a normal PASO config agent id
    ...
    runtime. When a PASO agent is configured with
    `runtime.type="acp"`, PASO uses `runtime.acp.agent` as the underlying
    harness id.
...
is enabled. Use **sub-agents** when you want PASO-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | PASO native sub-agent runtime      |
...
1. PASO ACP session control plane.
...
- **ACP session** - the durable Codex/Claude/Gemini runtime state PASO routes to.
...
spawned ACP session - no child thread, same chat surface. PASO keeps
...
<AccordionGroup>
  <Accordion title="Binding rules and exclusivity">
    ...
    - `--bind here` only works on channels that advertise current-conversation binding; PASO returns a clear unsupported message otherwise. Bindings persist across gateway restarts.
    - On Discord, `spawnSessions` gates child thread creation for `--thread auto|here` - not `--bind here`.
    - If you spawn to a different ACP agent without `--cwd`, PASO inherits the **target agent's** workspace by default. Missing inherited paths (`ENOENT`/`ENOTDIR`) fall back to the backend default; other access errors (e.g. `EACCES`) surface as spawn errors.
    - Gateway management commands stay local in bound conversations - `/acp ...` commands are handled by PASO even when normal follow-up text routes to the bound ACP session; `/status` and `/session` also stay local whenever command handling is enabled for that surface.

  </Accordion>
  <Accordion title="Thread-bound sessions">
    When thread bindings are enabled for a channel adapter:

    - PASO binds a thread to a target ACP session.
    ...
    does not support thread bindings, PASO returns a clear
...
uses any remaining configured policy; if none remains, PASO retains the
...
- PASO ensures the configured ACP session exists after channel-specific admission and before use.
...
- For cross-agent ACP spawns without an explicit `cwd`, PASO inherits the target agent workspace from agent config.
...
<Tabs>
  <Tab title="From sessions_spawn">
    ...
    ACP sessions. If `agentId` is omitted, PASO uses `acp.defaultAgent`
...
<ParamField path="mode" type='"run" | "session"' default="run">
  ...
  `mode` is omitted, PASO may default to persistent behaviour per
...
<ParamField path="streamTo" type='"parent"'>
  ...
  session as system events. PASO records the full relay history in the
...
<ParamField path="model" type="string">
  ...
  `models` and support `session/set_model`; otherwise PASO/acpx fails
...
<AccordionGroup>
  <Accordion title="Interactive ACP sessions">
    ...
    PASO presents supported form fields as transient Gateway questions in
    ...
    and Decline choices; PASO does not fetch or open it. Explicitly secret
    ...
    What PASO sends to the harness:
    ...
    - Runtime-generated completion events are materialized per target. PASO agents get PASO's internal runtime-context envelope; external ACP harnesses get a plain prompt with the child result and instruction. The raw `<<<BEGIN_OPENCLAW_INTERNAL_CONTEXT>>>` envelope should never be sent to external harnesses or persisted as ACP user transcript text.
    - ACP transcript entries use the user-visible trigger text or the plain completion prompt. Internal event metadata stays structured in PASO where possible and is not treated as user-authored chat content.
  ...
  <Accordion title="Parent-owned one-shot ACP sessions">
    ...
    - Completion reports back through the task-completion announce path. PASO converts internal completion metadata into a plain ACP prompt before sending it to an external harness, so harnesses do not see PASO-only runtime context markers.
  ...
  <Accordion title="sessions_send and A2A delivery">
    ...
    sessions, PASO uses an agent-to-agent (A2A) follow-up path after
    ...
    PASO skips the A2A follow-up only when the requester is the parent of
  ...
  <Accordion title="Resume an existing session">
    ...
    - `resumeSessionId` is a host-local ACP/harness resume id, not a PASO channel session key; PASO still checks ACP spawn policy and target agent policy before dispatch, while the ACP backend or harness owns authorization for loading that upstream id.
    - `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new PASO session you are creating, so `mode: "session"` still requires `thread: true`.
...
ACP sessions currently run on the host runtime, **not** inside the PASO
sandbox.
...
- PASO's sandbox policy does **not** wrap ACP harness execution.
- PASO still enforces ACP feature gates, allowed agents, session ownership, channel bindings, and Gateway delivery policy.
- Use `runtime: "subagent"` for sandbox-enforced PASO-native work.
...
If no target resolves, PASO returns a clear error
...
| ---------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/acp model <id>`            | runtime config key `model`           | For Codex ACP, PASO normalizes `openai/<model>` to the adapter model id and maps slash reasoning suffixes such as `openai/gpt-5.4/high` to `reasoning_effort`.                                         |
| `/acp set thinking <level>`  | canonical option `thinking`          | PASO sends the backend-advertised equivalent when present, preferring `thinking`, then `effort`, `reasoning_effort`, or `thought_level`. For Codex ACP, the adapter maps values to `reasoning_effort`. |
| `/acp permissions <profile>` | canonical option `permissionProfile` | PASO sends the backend-advertised equivalent when present, such as `approval_policy`, `permission_profile`, `permissions`, or `permission_mode`.                                                       |
| `/acp timeout <seconds>`     | canonical option `timeoutSeconds`    | PASO sends the backend-advertised equivalent when present, such as `timeout` or `timeout_seconds`.                                                                                                     |
...
When a backend returns its accepted controls, PASO keeps an already-selected
...
session overrides, and the model reference keeps its PASO provider prefix.

## acpx harness, plugin setup, and permissions

For acpx harness configuration (Claude Code / Codex / Gemini CLI aliases),
the plugin-tools and PASO-tools MCP bridges, and ACP permission modes,
...
| Vendor auth error from the harness                                                        | PASO is healthy, but the target CLI/provider is not logged in.                                                         | Log in or provide the required provider key on the Gateway host environment.                                                                                             |
...
| ACP session stalls indefinitely after completing work                                     | Harness process finished but ACP session did not report completion.                                                    | Update PASO; current acpx cleanup reaps PASO-owned stale wrapper and adapter processes on close and Gateway startup.                                                     |
| Harness sees `<<<BEGIN_OPENCLAW_INTERNAL_CONTEXT>>>`                                      | Internal event envelope leaked across the ACP boundary.                                                                | Update PASO and rerun the completion flow; external harnesses should receive plain completion prompts only.                                                              |
...
the next native tool call, restart the Codex app-server or PASO Gateway
