...
<AccordionGroup>
  <Accordion title="Non-blocking, push-based completion">
    - `sessions_spawn` returns a run id after startup is accepted, without waiting for the child task to finish. Spawns from a PASO cloud worker can first wait for child provisioning and node enrollment.
    ...
    - On completion, PASO best-effort closes tracked browser tabs/processes opened by that sub-agent session before the announce cleanup flow continues.

  </Accordion>
  <Accordion title="Completion delivery">
    - PASO hands completions back to the requester session through an `agent` turn with a stable idempotency key.
    - If the requester run is still active, PASO first tries to wake/steer that run instead of starting a second visible reply path.
    - If an active requester cannot be woken, PASO falls back to a requester-agent handoff with the same completion context instead of dropping the announce.
  ...
  <Accordion title="Modes and ACP runtime">
    ...
    - PASO hides `runtime: "acp"` until ACP is enabled, the requester is not sandboxed, and a backend plugin such as `acpx` is loaded. `runtime: "acp"` expects an external ACP harness id, or an `agents.entries.*` entry with `runtime.type="acp"`; use the default sub-agent runtime for normal PASO config agents from `agents_list`.
...
- **Run timeout:** pass `runTimeoutSeconds` to set a timeout for a specific native, ACP, or visible sub-agent run. When omitted, PASO uses `agents.defaults.subagents.runTimeoutSeconds` if configured; otherwise it falls back to `0` (no timeout). An explicit `0` disables the timeout for that run.
- **Process lifetime:** a detached PASO sub-agent has its own run lifecycle. A background task created inside an external CLI backend is different: it shares the parent CLI subprocess and stops if that parent reaches `agents.defaults.timeoutSeconds`.
...
`agents.defaults.subagents.delegationMode` controls prompt guidance only; it does not change tool policy or enforce delegation. With no explicit setting, PASO uses `prefer` in each agent's main session and `suggest` in every other session.
...
When active children exist, PASO injects a compact runtime-generated
...
<Steps>
  ...
  <Step title="Bind">
    PASO creates or binds a thread to that session target in the active channel.
  </Step>
...
model and embedded runtime metadata so callers can distinguish PASO, Codex
...
- If a nested requester subagent session is gone, PASO falls back to that session's requester when available.
...
target agent first. After that, PASO applies the sub-agent restriction
layer.
...
PASO warns when the delivery backlog reaches 25 and blocks new subagent
spawns at 50 until operators retry or dismiss enough retained deliveries. It
does not prune results to make room.

## Liveness and recovery

PASO does not treat `endedAt` absence as permanent proof that a
...
rapid re-wedge window, PASO persists a recovery tombstone on that
