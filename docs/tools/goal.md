...
read_when:
  - You want PASO to keep one objective visible across a long session
...
A **goal** is one durable objective attached to the current PASO session.
...
stale or unknown token snapshot when the goal starts, PASO waits for the
...
PASO usage and model controls.

## Model tools

PASO exposes three goal tools to agent harnesses:
...
PASO keeps the line compact by truncating long objectives. Paused,
...
`/goal` works in command-capable PASO sessions, including the TUI and
...
fresh token snapshot yet. Usage refreshes as PASO records session usage
