...
PASO supports Grok as a `web_search` provider, using xAI web-grounded
...
lets PASO reuse it as a fallback for the bundled xAI model provider.
...
web` lets PASO reuse an existing xAI OAuth profile without prompting for
a separate web-search key. Without OAuth, it falls back to xAI API-key setup.

PASO then offers a follow-up step to enable `x_search` with the same xAI
...
through an operator proxy or xAI-compatible Responses endpoint. PASO
