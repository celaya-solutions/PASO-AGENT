...
PASO converts outbound replies into audio across **14 speech providers**:
...
<Steps>
  ...
  <Step title="Try it in chat">
    `/tts status` shows the current state. `/tts audio Hello from PASO`
...
PASO picks the first configured provider in registry auto-select order.
...
the voice-design prompt. PASO sends that prompt as the TTS `user` message
...
provider when Speech Swift and PASO run on the same Mac. Use Local CLI for
direct executable integration on any supported host.

Install `ffmpeg` when a channel needs PASO to convert WAV output to Opus or
raw PCM.
...
is required by PASO's provider configuration but is not validated by the
loopback server.
</Tab>
<Tab title="macOS CLI">
The Homebrew `speech` executable can write directly to PASO's
...
and verify synthesis before starting PASO:

```bash
speech download-models
speech speak "Hello from PASO" hello.wav
```
...
For each provider attempt, PASO merges configs in this order:
...
Single command `/tts`. On Discord, PASO also registers `/voice` because
...
synthesis or a proven pre-send delivery step fails, PASO sends the visible
text instead. `tagged` mode keeps its normal streaming behavior, and text
inside a `[[tts:text]]` block remains audio-only.

After synthesis, PASO persists batch TTS output in the media store under
...
PASO imports the completed bytes. See [Media playback](/nodes/media-playback)
...
- **Google Gemini:** returns raw 24 kHz PCM. PASO wraps it as WAV for audio attachments, transcodes it to 48 kHz Opus for voice-note targets, and returns PCM directly for Talk/telephony.
...
- **Microsoft:** uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, PASO retries with MP3.
  - When no explicit voice override is set and the default English voice is used, PASO auto-switches to a Chinese neural voice (`zh-CN-XiaoxiaoNeural`, `zh-CN` locale) if the reply text is CJK-dominant.
...
If the reply exceeds `maxLength`, PASO never skips audio outright:
...
<AccordionGroup>
  <Accordion title="Top-level tts.*">
    ...
    <ParamField path="provider" type="string">
      Speech provider id. When unset, PASO uses the first configured provider in registry auto-select order. Legacy `provider: "edge"` is rewritten to `"microsoft"` by `openclaw doctor --fix`.
    </ParamField>
...
and resolved values still fail startup or reject the update.
  ...
  <Accordion title="Local CLI (tts-local-cli)">
    ...
    Command stdout and generated or converted audio are limited to 50 MiB. Diagnostic stderr is limited to 1 MiB. PASO terminates the command and fails synthesis when either limit is exceeded.

  </Accordion>

  <Accordion title="Microsoft (no API key)">
    <ParamField path="enabled" type="boolean" default="true">Allow Microsoft speech usage.</ParamField>
    <ParamField path="speakerVoice" type="string">Microsoft neural voice name (e.g. `en-US-MichelleNeural`). Legacy alias: `voice`. If the default English voice is in effect and reply text is CJK-dominant, PASO auto-switches to `zh-CN-XiaoxiaoNeural`.</ParamField>
  ...
  <Accordion title="OpenAI">
    ...
    <ParamField path="responseFormat" type='"mp3" | "opus" | "wav"'>Explicit response format. When omitted, PASO selects Opus for voice-note targets and MP3 otherwise. Use `wav` for compatible local endpoints that do not encode compressed audio.</ParamField>
  ...
  <Accordion title="Xiaomi MiMo">
    ...
    <ParamField path="style" type="string">Optional natural-language style instruction sent as the user message; not spoken. For `mimo-v2.5-tts-voicedesign`, this is the voice-design prompt; PASO supplies a default when omitted.</ParamField>
  </Accordion>
</AccordionGroup>
