...
generated audio is still missing from the reply, PASO sends an
...
<Note>
Not all providers support all parameters. PASO still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, PASO
...
Provider request timeouts are operator configuration only. PASO uses
...
- **Completion wake:** PASO injects an internal completion event back
