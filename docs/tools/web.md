...
normalized results, cached by query for 15 minutes (configurable). PASO
...
<Steps>
  ...
  <Step title="Configure">
    ...
    [PASO](/cli/openclaw): say `configure web search` in `openclaw setup`
    or in the Control UI's **Settings → Ask PASO** chat. The hosted flow
  ...
  <Step title="Use it">
    ```javascript
    await web_search({ query: "PASO plugin SDK" });
    ```
...
no `provider` is set, PASO checks providers in this order and uses the
...
`openclaw configure --section web`. PASO does not send managed
...
<Note>
  ...
  selected through auto-detect. In auto-detect mode, PASO resolves only the
...
`web_search` tool automatically when PASO web search is enabled and no
...
search and PASO's managed `web_search` dynamic tool are mutually exclusive,
so managed search cannot bypass native domain restrictions. PASO uses the
...
- Set `tools.web.search.provider` to a managed provider such as `brave` to use
  PASO's managed `web_search` instead
...
use the managed `web_search` fallback through PASO's dynamic tool namespace.
Use an explicit managed provider when you need PASO's provider-specific
...
Managed HTTP `web_search` provider calls use PASO's guarded fetch path,
scoped to the current provider's own hostname. For that hostname only,
PASO allows Surge, Clash, and sing-box fake-IP DNS answers in
...
`tools.web.search.cacheTtlMinutes` controls PASO's local search-result
...
PASO keeps startup resilient and reports a warning so you can reinstall the
...
- or omit that field and let PASO auto-detect the first ready web-fetch
...
`openclaw configure --section web`, PASO can also ask for:
...
PASO also offers optional `x_search` setup with the same credential right
...
provider, PASO does not show the `x_search` prompt.
...
optional structured filters. PASO constructs the built-in xAI `x_search`
...
`plugins.entries.xai.config.xSearch.cacheTtlMinutes` controls PASO's local
