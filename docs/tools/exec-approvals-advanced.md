...
- If PASO cannot identify exactly one concrete local file for an interpreter/runtime command
...
session, PASO also resumes that session with an internal followup so the agent observes that
...
After an approved async exec finishes, PASO sends a followup `agent` turn to the same session.
...
channel, PASO keeps the local deterministic `/approve` prompt visible. If the native runtime is
active and attempts delivery but no target receives the card, PASO sends a same-chat fallback
...
- Telegram defaults to approver DMs (`target: "dm"`). Switch to `channel` or `both` to also show
  approval prompts in the originating Telegram chat/topic. For Telegram forum topics, PASO
...
- Matrix native DM/channel routing and reaction shortcuts handle both exec and plugin approvals;
  ...
  include `com.openclaw.approval` custom event content on the first prompt event so PASO-aware
...
owner routing for approval prompts and final results. PASO first tries a private route on the
