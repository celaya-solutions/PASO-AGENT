...
read_when:
  - Running PASO Gateway in WSL2 while Chrome lives on Windows
...
In the common split-host setup, PASO Gateway runs inside WSL2, Chrome runs
...
- PASO points a browser profile at the address reachable from WSL2
...
```powershell
chrome.exe --remote-debugging-port=9222 --user-data-dir="$env:LOCALAPPDATA\PASO\ChromeCDP"
```
...
If this fails, diagnose the Windows listeners below. PASO is not the
problem yet.
...
that before touching PASO config.

### Layer 3: configure the correct browser profile

Point PASO at the address reachable from WSL2:
...
- use HTTP(S) when you want PASO to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools
  socket URL
- test the same URL with `curl` before expecting PASO to succeed
...
3. PASO config: does `browser.profiles.<name>.cdpUrl` use that exact
...
from WSL2 second, and only then debug PASO config or Control UI auth.
