---
summary: "Deploy PASO on Render with Infrastructure-as-Code"
read_when:
  - Deploying PASO to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy PASO on [Render](https://render.com) using the repo's `render.yaml` Blueprint. It declares the service, disk, and environment variables in one file.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/celaya-solutions/PASO-AGENT)
...
The Blueprint defaults to `starter`. To use the free tier, change `plan: free` **and delete the `disk:` block** in your fork's `render.yaml`; Render rejects a Blueprint that attaches a persistent disk to a free instance. Without that disk, PASO state resets on every deploy.
...
Render redeploys automatically when the connected PASO repository branch gets a new commit. Review upstream framework changes before bringing them into the PASO fork, then let Render deploy the reviewed fork revision.
...
- **Vertical**: change the plan for more CPU/RAM. Usually sufficient for PASO.
- **Horizontal**: increase instance count (Standard plan and above). Requires sticky sessions or external state management since PASO keeps runtime state on the local disk.
