---
summary: "Deploy the PASO source repository on Railway"
read_when:
  - Deploying PASO to Railway
  - You want a hosted Gateway with browser-based Control UI
title: "Railway"
---

Deploy PASO on Railway from the Celaya Solutions Research source repository and access it through the web Control UI.

## Deploy from the PASO repository

<Steps>
  <Step title="Create the Railway service">
    In Railway, choose **New Project → Deploy from GitHub repo** and select [celaya-solutions/PASO-AGENT](https://github.com/celaya-solutions/PASO-AGENT). Railway should detect the repository's `Dockerfile`.
  </Step>
