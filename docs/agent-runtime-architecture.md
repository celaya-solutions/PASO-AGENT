...
summary: "How PASO structures the built-in agent runtime: code layout, boundaries, resource manifests, and runtime selection."
---

PASO owns the built-in agent runtime. Runtime code lives under `src/agents/`, model/provider transport lives under `src/llm/`, and plugin-facing contracts are exposed through `openclaw/plugin-sdk/*` barrels.
...
| `src/agents/runtime/`               | PASO facade that wires `@openclaw/agent-core` to the plugin SDK LLM runtime and re-exports it plus local proxy utilities.                                                                                                 |
| `src/agents/agent-tools*.ts`        | PASO-owned tool definitions, parameter schemas, tool policy, before/after tool-call adapters, and host/sandbox edit tools.                                                                                                |
...
Core calls the built-in runtime through PASO modules and SDK barrels; no external agent framework packages remain. Plugins use documented `openclaw/plugin-sdk/*` entrypoints and do not import `src/**` internals.
...
Resource packages declare PASO resources in `package.json` metadata. Entries are file paths or globs relative to the package root:
...
- `auto` selects a registered plugin harness that supports the effective provider route, otherwise the built-in PASO runtime. A provider or model prefix alone never selects a harness.
...
- [PASO agent runtime workflow](/openclaw-agent-runtime)
