...
PASO can run a local AI CLI as a text-only fallback when API providers are down, rate-limited, or misbehaving. It is intentionally conservative:

- PASO tools are not injected directly, but a backend with `bundleMcp: true` can receive gateway tools through a loopback MCP bridge.
...
PASO auto-loads an owning bundled plugin when model selection or a
...
Credentials remain in PASO auth profiles or the owning plugin's config.
...
2. Builds a system prompt using the same PASO prompt and workspace context.
...
Background work started inside a CLI is still part of that CLI subprocess. If the parent turn reaches its overall limit, PASO stops the subprocess and its CLI-internal background tasks together. For durable long work, use a detached PASO [sub-agent](/tools/subagents) or [ACP agent](/tools/acp-agents); detached sub-agents have no run timeout by default.
...
subscription. PASO uses a non-secret route marker. It never reads, persists,
...
receives it as a native hook attachment, while PASO history preserves the original user message. The
...
claude --version
claude update
# Restart the PASO gateway after updating.
```

The bundled `claude-cli` backend prefers Claude Code's native skill resolver. When the current skills snapshot has at least one selected skill with a materialized path, PASO passes a temporary Claude Code plugin via `--plugin-dir` and omits the duplicate PASO skills catalog from the appended system prompt. Without a materialized plugin skill, PASO keeps the prompt catalog as a fallback. Skill env/API key overrides still apply to the child process environment for the run.

The Agent SDK always runs with Claude Code's default permission mode.
PASO's SDK permission callback and `PreToolUse` hook keep native tools under
...
arguments projected into their PASO equivalents. Per-agent and session
restrictions still override broader global policy. PASO-owned MCP tools
...
When the effective exec ask setting is `on-miss` or `always`, PASO relays
...
- Fresh CLI sessions normally reseed from PASO's latest compaction summary, the messages retained by that compaction, and subsequent turns on the active branch. PASO reads this history from the canonical session SQLite database; it does not require a PASO JSONL transcript file. To recover short sessions invalidated before compaction, a backend can opt in with `reseedFromRawTranscriptWhenUncompacted: true`. Raw transcript reseed stays bounded and limited to safe invalidations, such as a missing CLI transcript, an orphaned tool-use tail, message-policy/system-prompt/cwd/MCP changes, or a session-expired retry; auth profile or credential-epoch changes never reseed raw transcript history.

Serialization: `serialize: true` keeps same-lane runs ordered (most CLIs serialize on one provider lane). PASO also drops stored CLI session reuse when the selected auth identity changes, including a changed auth profile id, static API key, static token, or OAuth account identity when the CLI exposes one; OAuth access/refresh token rotation alone does not cut the session. If a CLI has no stable OAuth account id, PASO lets that CLI enforce its own resume permissions.

## Fallback prelude from claude-cli sessions

When a `claude-cli` attempt fails over to a non-CLI candidate in [`agents.defaults.model.fallbacks`](/concepts/model-failover), PASO seeds the next attempt with a context prelude harvested from Claude Code's local JSONL transcript (under `~/.claude/projects/`, keyed per workspace). This supplies CLI-owned context that may not be present in PASO's SQLite session transcript.
...
PASO writes base64 images to temp files. If `imageArg` is set, those paths are passed as CLI args; if not, PASO appends the file paths to the prompt (path injection), which works for CLIs that auto-load local files from plain paths.
...
- For Gemini CLI JSON output, PASO reads reply text from `response` and usage from `stats` when `usage` is missing or empty. The bundled Gemini CLI adapter uses `stream-json`.
...
Anthropic owns `claude-cli` and Google owns `google-gemini-cli`. OpenAI Codex agent runs use the Codex app-server harness through `openai/*`; PASO no longer registers a bundled `codex-cli` backend.
...
valid legacy Gemini CLI OAuth profiles remain runtime-compatible, but PASO
...
- Usage falls back to `stats` when `usage` is absent or empty; `stats.cached` normalizes into PASO `cacheRead`, and if `stats.input` is missing, input tokens derive from `stats.input_tokens - stats.cached`.
...
Some CLI backends run an agent that compacts its own transcript, so PASO must not run its safeguard summarizer against them — doing so fights the backend's own compaction and can hard-fail the turn.

`claude-cli` has no harness endpoint (Claude Code compacts internally), so it declares `ownsNativeCompaction: true`. Automatic PASO compaction defers to Claude Code, while an explicit `/compact` resumes the bound Claude Code session and sends its native `/compact` command. PASO passes the run's effective context budget through Claude Code's documented [`CLAUDE_CODE_AUTO_COMPACT_WINDOW`](https://code.claude.com/docs/en/env-vars), keeping native auto-compaction aligned with configured Anthropic `contextTokens` limits. Native-harness sessions such as Codex keep routing to their harness compaction endpoint instead.

`google-gemini-cli` also owns automatic compaction and persists its compressed session for resume. PASO defers to Gemini CLI rather than running a second summarizer. Explicit `/compact` is unsupported for this backend because it does not declare a manual compaction capability.
...
Add the atomic `manualCompaction` capability only when its command compacts the resumed session in place. Its `input` selects the transport the backend command actually recognizes, and `validateOutput` must require a positive backend acknowledgement rather than treating a zero exit as success. PASO runs it as an internal control operation: it is not written as a user turn and does not run agent or context-engine turn hooks.

## Bundle MCP overlays

CLI backends do not receive PASO tool calls directly, but a backend can opt into a generated MCP config overlay with `bundleMcp: true`. Current bundled behavior:

- `claude-cli`: generated strict MCP config file.
- `google-gemini-cli`: generated Gemini system settings file.

When bundle MCP is enabled, PASO:
...
PASO lists each server through its session-scoped runtime, assigns the same
...
operator policy in PASO configuration.
...
PASO tool through the grant-scoped MCP server. This keeps filesystem,
process, exec, approval, and sandbox policy inside PASO instead of widening
...
If no MCP servers are enabled, PASO still injects a strict config when a backend opts into bundle MCP, so background runs stay isolated.
...
login. PASO uses a non-secret route marker and never reads, persists,
...
Explicit PASO-managed API-key and token profiles continue to use the
protected, per-invocation credential-forwarding CLI path.

## Reseed history cap

When a fresh CLI session is seeded from a prior PASO transcript (for example after a `session_expired` retry), the rendered `<conversation_history>` block is capped to keep reseed prompts from exploding. The default is 12,288 characters (about 3,000 tokens).
...
- PASO does not inject tool calls into the CLI backend protocol. Backends only see gateway tools when they opt into `bundleMcp: true`.
