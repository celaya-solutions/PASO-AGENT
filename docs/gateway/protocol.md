...
PASO. Operator and node clients (CLI, web UI, macOS app, iOS/Android nodes,
...
(PASO wraps it via the thin `src/gateway/client.ts` facade). These
...
- PASO also has a narrow backend/container-local self-connect path for
...
- Omitting device identity has scope consequences. When a device-less
  operator connection is allowed through an explicit trust path, PASO
