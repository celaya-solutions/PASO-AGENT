---
summary: "Use the 1Password plugin, bundled skill, or official MCP with PASO"
read_when:
...
PASO pairs with **1Password** in four independent ways:
...
- **Browser sign-in:** the `claude-cli` backend can use Claude Code's Chrome integration with [1Password for Claude](https://support.1password.com/1password-claude/), letting the agent sign in to websites without the password ever reaching the model or PASO.
...
registered PASO credential targets, bounds parallel `op read` calls, and
...
PASO bundles a `1password` skill that teaches agents to select an available
...
into a one-off command. When a credential is already wired to a PASO
SecretRef target, the owning PASO workflow should resolve it; the agent does
...
and the PASO plugin does not call it. If an MCP-managed Environment launches
PASO with variables already mounted, use PASO's `env` SecretRefs for
...
[1Password for Claude](https://support.1password.com/1password-claude/) lets Claude request a login while the 1Password browser extension fills the credential directly into the page over an encrypted channel. The secret never enters the model context, the transcript, or PASO. When PASO runs the [`claude-cli` backend](/gateway/cli-backends#claude-cli-specifics) with Claude Code's Chrome integration enabled, agent tasks can use that flow for websites that need a real signed-in session.
...
- A person at the gateway host: every credential use shows a 1Password prompt confirmed there (for example with Touch ID). Under a restrictive exec policy the browser tool calls themselves are also relayed to your channel as PASO approvals first.

Before wiring this into PASO, verify the pieces in an interactive session on the gateway host: run `claude --chrome`, confirm the extension connects, and check that the `claude-in-chrome` tools include the credential tools. If they do not appear there, they will not appear through PASO either.
...
- Never place secret values in `openclaw.json`, logs, or chat. Scope the service
  account to only the vaults and items PASO needs.
