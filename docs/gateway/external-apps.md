...
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to PASO
...
External apps talk to PASO through the Gateway protocol: WebSocket
...
<Note>
  This page is for code outside the PASO process. Plugin code that runs
  inside PASO should use documented `openclaw/plugin-sdk/*` subpaths instead.
</Note>
...
4. Pin the PASO version you test against.
5. Recheck the RPC reference when upgrading PASO.
...
fail-closed, and the host must not freeze or snapshot it. PASO retries the
...
Without `drain: true`, preparation remains refuse-only: PASO closes new
...
<Tip>
  For host wake scheduling, keep the PASO-facing part in an in-process
...
Use Gateway RPC when code lives outside PASO:
...
Use the Plugin SDK when code runs inside PASO:
