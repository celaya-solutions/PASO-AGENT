...
summary: "Architecture and product boundary for supervising native Codex sessions from PASO."
read_when:
...
Codex supervision lets a PASO operator discover native Codex sessions,
create a Gateway-local branch, or continue an eligible paired-node thread
through the normal PASO Chat surface.
Codex App Server remains the thread and model-loop owner. PASO supplies the
...
- native thread bindings to PASO sessions
...
Codex Desktop -> private stdio App Server -> user Codex home
                                             ^
PASO Codex plugin -> supervision App Server connection
...
Ordinary PASO Codex sessions -> managed agent-home stdio by default
  -> ordinary full harness threads -> PASO Chat and channel delivery
```
...
process-local; a thread unknown to PASO's supervision process is `notLoaded`
...
unbounded `thread/read`. PASO also rejects any serialized item page above
...
eventually discovered without importing them into PASO storage.

Catalog discovery is passive. Listing or reading metadata must not call
`thread/resume`, subscribe the PASO client to live thread requests, or
answer an approval.
...
2. Otherwise projects bounded user and assistant history through the source's
   ...
   PASO Chat and records a pending harness branch.
...
2. On that same connection, starts the canonical full Codex harness thread with
   `threadSource: "appServer"`, PASO's cwd, policy, config, environment, the
   full PASO harness tool surface, and exactly the model and provider
...
omit PASO model and provider overrides, so Codex restores the persisted pair.
If a separate native Codex control changes the canonical thread, PASO accepts
that native persisted selection. The outer PASO model and fallback chain
...
Deleting or resetting the PASO entry would otherwise discard the native
...
canonical harness thread on the first turn prevents PASO from becoming a
...
Atlas, or ChatGPT source remains eligible for both native and PASO catalogs.
...
known active PASO binding owner for the exact target or any non-archived
spawned descendant also rejects archive. PASO paginates Codex's experimental
...
the owner/admin check and are blocked while PASO sandboxing is active.

The node runner rejects overlapping PASO resume turns for the same thread
...
- audited handoff between Codex Desktop, CLI, and PASO
...
- Later resumes omit PASO model/provider overrides, preserve Codex's
  canonical persisted selection, accept separate native changes to that thread,
  and never substitute the outer PASO model or fallback chain.
