...
- The SQLite transcript rows are the durable model/runtime transcript. For normal agent runs, the embedded PASO runtime persists model-visible `user`, `assistant`, and `toolResult` messages through the session accessor. WebChat does not write arbitrary delivery, status, or helper text into that transcript.
