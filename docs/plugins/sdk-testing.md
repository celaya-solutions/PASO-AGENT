---
summary: "Testing utilities and patterns for PASO plugins"
...
Reference for test utilities, patterns, and lint enforcement for PASO
plugins.
...
These subpaths are repo-local source entrypoints for PASO's own bundled
...
| `createOpenClawTestState` / `withOpenClawTestState` / `OpenClawTestState` | Create and clean up isolated PASO state, config, workspace, environment, and auth-profile fixtures. Import from `plugin-sdk/test-state`  |
...
Core-only suites that depend on bundled PASO inventory stay under
...
exercise PASO's loader acceptance gates. Add at least one loader-backed
...
PASO uses Vitest 4 with informational V8 coverage reporting. For plugin tests:
