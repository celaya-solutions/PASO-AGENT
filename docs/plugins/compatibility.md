...
read_when:
  - You maintain a PASO plugin
  - You see a plugin compatibility warning
  - You are planning a plugin SDK or manifest migration
---

PASO keeps older plugin contracts wired through named compatibility
...
PASO should not remove a documented plugin contract in the same release
...
The plugin inspector should live outside the core PASO repo as a
...
machine-readable output in CI annotations. PASO core should expose
...
lane when validating the external inspector against PASO plugin
packages. Run it from a clean PASO checkout after the package is built:
