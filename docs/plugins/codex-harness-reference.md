...
`~/.codex`) and the Codex home of every configured PASO agent. Register
additional local Codex stores only when sessions live in a home PASO does
...
and the selected PASO agent still owns the resulting connection; `homes`
...
resumes omit PASO model and provider overrides so Codex restores the
...
PASO binding owns the exact target or one of its non-archived spawned
descendants. PASO follows Codex's descendant pagination and fails closed on
...
For ordinary harness turns, PASO starts the managed Codex binary shipped
...
enables native Computer Use. If no desktop app bundle is installed, PASO
falls back to the pinned package binary.

Before cutting over a staged PASO package, run the opt-in managed-binary
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after PASO builds its inherited environment.                                                                                                                                                                                                                                                                                                    |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start and resume. Active PASO sandboxes narrow `danger-full-access` turns to Codex `workspace-write`; the turn network flag follows PASO sandbox egress.                                                                                                                                                                                                                                  |
...
sends `null` to clear the PASO-owned tier, and auto decides for each model
...
the shared PASO session policy. These values describe native configuration
...
contract. When enabled, PASO also sets `features.network_proxy.enabled` and
`default_permissions` in the Codex thread config so the generated permission
profile can start Codex-managed networking. PASO generates a
...
PASO treats non-loopback WebSocket app-server URLs as remote and requires
...
shorthand before PASO builds app-server start options, and unresolved
structured SecretRefs fail before any token or header is sent.

When native Codex plugins are configured, PASO caches one
...
authenticated metadata for at most 100 requested app IDs per call. PASO
...
PASO provisionally admits only ownership-proven, policy-approved apps,
...
Attestation completes before PASO injects history, starts a turn, or
persists the native thread binding. On failure, PASO deletes a persistent
...
PASO uses its `plugin/installed` snapshot and reads only the exact
...
Only connect PASO to a `0.149.0` or newer remote app-server trusted to accept
...
unattended PASO turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, PASO treats the implicit default as guardian
...
When a PASO sandbox is active, the local Codex app-server process still
runs on the Gateway host. PASO therefore disables Codex native Code Mode,
user MCP servers, and app-backed plugin execution for that turn instead of
treating Codex host-side sandboxing as equivalent to the PASO sandbox
backend. Shell access is exposed through PASO sandbox-backed dynamic tools
such as `sandbox_exec` and `sandbox_process` when the normal exec/process tools
are available.

<Note>
On Docker-backed PASO sandbox hosts (`agents.defaults.sandbox.mode` set to
...
Ubuntu/AppArmor hosts. Fix the reported host namespace policy for the PASO
...
The stable default is fail-closed: active PASO sandboxing disables native
...
to try Codex's remote environment support with PASO's sandbox backend.
...
When the flag is on and the current PASO session is sandboxed, PASO
...
PASO-owned environment. If the app-server cannot register the environment,
...
notifications. PASO retains only a bounded recent-output buffer for polling
...
the loopback exec-server unless it is running on the same host, so PASO
...
PASO worker slot. HTTP requests containing authentication, cookies, API
...
subcommand, PASO groups `-c` / `--config` overrides before that subcommand,
...
sides of `app-server`. PASO's ephemeral credential-store override remains
last when PASO owns auth; native user-home auth is unchanged.
...
PASO supplies auth in this order:

1. An explicit or ordered PASO auth profile for the agent.
...
Token refresh is inverted so the long-lived secret never leaves PASO. Codex
holds only a short-lived access token, and on an unauthorized response it sends
an `account/chatgptAuthTokens/refresh` request back to PASO over the same
connection. PASO refreshes against its own auth profile store and returns a
...
When PASO sees a ChatGPT subscription-style Codex auth profile (OAuth or
...
Stdio app-server launches inherit PASO's process environment by default.
PASO owns the Codex app-server account bridge and sets `CODEX_HOME` to a
per-agent directory under that agent's PASO state. That keeps Codex
config, accounts, plugin cache/data, and thread state scoped to the PASO
...
PASO skips its auth-profile bridge for the app-server. Verified owner
...
continuing it in PASO; independent Codex processes do not coordinate
concurrent writers for the same thread.

That `homeScope` opt-in applies to ordinary harness sessions. Hosted web search
and settled-turn finalization use private temporary homes and PASO auth
...
unrelated thread not owned by another PASO Chat require
`allowWriteControls`. Neither option bypasses a locked binding.

PASO does not rewrite `HOME` for normal local app-server launches.
...
In the default agent scope, PASO plugins and PASO skill snapshots
still flow through PASO's own plugin registry and skill loader; personal
Codex `~/.codex` assets do not. If you have useful Codex CLI skills or
plugins from a Codex home that should become part of an isolated PASO
...
PASO removes `CODEX_HOME` and `HOME` from this list during local launch
...
`openclaw` namespace with `deferLoading: true`. PASO normally does not
...
available through the PASO dynamic-tool bridge as the durable session status
surface.

When a finite runtime allowlist disables native Code Mode, PASO sends an
empty execution-environment selection. In that direct, unsandboxed case,
PASO keeps its policy-filtered `exec` and `process` tools as the shell
fallback. Runtime allowlists and `codexDynamicToolsExclude` still apply.

Most remaining PASO integration tools, such as messaging, media, cron,
...
`sessions_spawn` remains available for explicit PASO or ACP delegation.
Message-tool-only source replies also stay direct, since that is a
turn-control contract.

Codex Code Mode projects generic PASO dynamic-tool results as text. Parse a
...
tool, are grouped under `openclaw_direct`. PASO adds that namespace to
...
structured wait result can reach Codex. On timeout, PASO aborts the tool
...
releases the PASO session lane so follow-up chat messages are not queued
...
post-tool idle budget: after PASO returns an `item/tool/call` response,
...
PASO best-effort interrupts the native turn and releases the session
...
PASO upgrades the bundled `@openai/codex` version or when a deployment
...
reads do not start discovery. Native configuration changes outside PASO
require the native owner's supported reload/restart and a catalog refresh;
PASO does not poll native home files for readiness. Authored host routes and
...
preserved in PASO model parameters. Renaming the picker label does not hide
...
The app-server catalog can report `ultra`; PASO reasoning controls for the
...
Codex-native capability outside the PASO policy boundary.
...
the harness applies the named PASO denial directly. Any other deny fails
...
PASO disables inherited and configured MCP servers, attests that they remain
...
agent path used by PASO setup and repair flows. The host must activate the
...
PASO's `AGENTS.md` developer-instruction carrier, so ambient workspace
...
PASO does not write synthetic Codex project-doc files or depend on Codex
...
filesystem environment, so PASO instead sends the bounded workspace
...
For PASO workspace parity, local tool notes live in the `## Tools` section
...
- `SOUL.md`, `IDENTITY.md`, and `USER.md` are forwarded as **turn-scoped**
  ...
  user profile.
- The compact loaded PASO skills list is also forwarded as turn-scoped
...
- `BOOTSTRAP.md`, when present, is forwarded as PASO turn input reference
  context.
