---
summary: "Run PASO embedded agent turns through the external GitHub Copilot SDK harness"
...
PASO's built-in harness. The Copilot CLI session owns the low-level
agent loop: native tool execution, native compaction (`infiniteSessions`), and
CLI-managed thread state under `copilotHome`. PASO still owns chat
...
- PASO with the `@openclaw/copilot` plugin installed.
...
- A writable `copilotHome` directory. Defaults to `<agentDir>/copilot` when
  PASO provides an agent directory, otherwise
...
[Quickstart](#quickstart). Without that opt-in, PASO uses its built-in
...
through PASO's guarded fetch path so DNS pinning and SSRF policy stay
owned by PASO. Use the native PASO runtime for local Ollama, LM
Studio, or LAN model servers.

## BYOK

Copilot BYOK uses the SDK's session-level custom provider contract. PASO
...
4. **Env-var fallback**, checked in this order (first non-empty value wins,
   ...
   1. `OPENCLAW_GITHUB_TOKEN` — harness-specific override; lets you pin a
      token for the PASO harness without disturbing system-wide `gh` /
      Copilot CLI config.
...
PASO's `models.json` / `openclaw-agent.sqlite`), or
...
| `reasoningEffort`        | `"low" \| "medium" \| "high" \| "xhigh"`. Maps from PASO's `ThinkLevel` / `ReasoningLevel` resolution in `auto-reply/thinking.ts`.                                                                                                                                                              |
| `infiniteSessionConfig`  | Optional override for the SDK `infiniteSessions` block driven by `harness.compact`. Safe to leave as-is.                                                                                                                                                                                        |
| `hooksConfig`            | Optional native Copilot SDK `SessionHooks` config for tool/MCP, user-prompt, session, and error callbacks. Separate from PASO's portable lifecycle hooks.                                                                                                                                       |
...
PASO plugin hooks need no Copilot-specific attempt configuration. The
...
`before_compaction` and `after_compaction`. Bridged PASO tools run
`before_tool_call` and report `after_tool_call`; `hooksConfig` remains for
native SDK-only callbacks with no portable equivalent.

Nothing else in PASO needs to know about these fields. Other plugins,
...
The PASO-side transcript journal (below) keeps receiving post-compaction
...
turn's messages into the PASO session transcript. Journal identity is
...
so PASO's `/btw` dispatcher (`src/agents/btw.ts`) falls through to the
...
- `ask_user` uses the provider-neutral gateway question runtime. The Control
  UI shows the same question card as other PASO questions, supported
...
Permission enforcement for bridged PASO tools happens **inside the tool
...
- `skipPermission: true` — tells the SDK not to fire
  ...
  wrapped `execute()` already performs the richer PASO policy check; an
  SDK-level prompt would either short-circuit PASO's enforcement
  (allow-all) or block every tool call (reject-all) — neither matches PI
  parity.

The in-tree Codex harness uses the same split: bridged PASO tools are
...
catalog executor instead of every PASO tool schema. When code mode is
...
when the SDK request allows them. Aborting the PASO attempt cancels the
