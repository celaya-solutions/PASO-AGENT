...
read_when:
  ...
  - You want PASO config credentials to resolve from 1Password
  - You need per-secret approval policy and audit history
  - You are configuring a 1Password service account for PASO
title: "1Password"
---
...
- Curated agent registry only. Agents can list configured slugs, but the plugin
  ...
  explicitly stored on registered PASO credential targets.
- Per-slug `auto`, `approve`, or `deny` policy.
- Approval grants expire. A cached value never bypasses current policy.
- Every access attempt is recorded in PASO's shared SQLite state. Audit
...
- After the current tool execution, PASO-owned transcript persistence
...
Create the token directory and file under the PASO state directory:
...
PASO's shared exec-id grammar in a plugin-local opaque form and decodes them
...
approval. PASO offers allow always only when the caller has a concrete agent
