...
The Policy plugin contributes doctor health checks for policy-managed PASO
...
posture, ingress/channel access posture, data-handling posture, and PASO config secret
provider/auth profile posture.

Policy stores authored requirements in `policy.jsonc`, observes existing
PASO settings and workspace declarations as evidence, and reports drift
