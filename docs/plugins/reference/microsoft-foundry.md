---
summary: "Adds Microsoft Foundry model provider support to PASO."
read_when:
...
Adds Microsoft Foundry model provider support to PASO.
...
- For Entra ID auth, install the Azure CLI and run `az login` before
  onboarding. PASO refreshes Microsoft Foundry runtime tokens through
...
PASO uses the Foundry `/openai/v1` endpoint for supported OpenAI-compatible
chat APIs:
...
Claude model ID, set `params.canonicalModelId` on the model entry so PASO
...
deployment through onboarding or include model metadata so PASO can verify
