---
summary: "Adds the iMessage channel surface for sending and receiving PASO messages."
read_when:
  - You are installing, configuring, or auditing the imessage plugin
title: "iMessage plugin"
---

# iMessage plugin

Adds the iMessage channel surface for sending and receiving PASO messages.

## Distribution

- Package: `@openclaw/imessage`
- Install route: npm; ClawHub: `clawhub:@openclaw/imessage`

## Surface

channels: `imessage`; skills

## Related docs

- [iMessage](/channels/imessage)
