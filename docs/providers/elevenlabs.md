---
summary: "Use ElevenLabs speech, Scribe STT, and realtime transcription with PASO"
read_when:
  - You want ElevenLabs text-to-speech in PASO
...
PASO uses ElevenLabs for text-to-speech, batch speech-to-text with Scribe
...
| Capability               | PASO surface                                                         | Default                  |
| ------------------------ | -------------------------------------------------------------------- | ------------------------ |
...
Set `modelId` to `eleven_v3` to use ElevenLabs v3 TTS. PASO keeps
...
returned audio stream instead of waiting for PASO to download the whole
audio file first. `latencyTier` maps to ElevenLabs' `optimize_streaming_latency`
query parameter for models that accept it; PASO omits that parameter for
...
PASO sends multipart audio to ElevenLabs `/v1/speech-to-text` with
