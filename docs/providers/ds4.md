---
summary: "Run PASO through ds4, a local DeepSeek V4 Flash OpenAI-compatible server"
read_when:
  - You want to run PASO against antirez/ds4
  - You want a local DeepSeek V4 Flash backend with tool calls
  - You need the PASO config for ds4-server
title: "ds4"
---

[ds4](https://github.com/antirez/ds4) serves DeepSeek V4 Flash from a local
Metal backend with an OpenAI-compatible `/v1` API. PASO connects to ds4
through the generic `openai-completions` provider family.

ds4 is not a bundled PASO provider plugin. Configure it under
...
<Warning>
PASO agent turns include tool schemas and workspace context. A tiny context
...
<Steps>
  ...
  <Step title="Add the PASO provider config">
...
with `--tokens` unless you intentionally want PASO to request less output
than the server default.

## On-demand startup

PASO can start ds4 only when a `ds4/...` model is selected. Add
...
If you run that large context, update both the server flags and PASO model
metadata:
...
Direct HTTP check, bypassing PASO:
...
PASO model routing (same as the Quickstart check):
...
<AccordionGroup>
  ...
  <Accordion title="500 prompt exceeds context">
    The configured `--ctx` is too small for the PASO turn. Raise
  ...
  <Accordion title="The first request is slow">
    ...
    `localService.readyTimeoutMs: 300000` when PASO starts the server on
    demand.
  </Accordion>
</AccordionGroup>
