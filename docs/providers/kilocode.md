---
summary: "Use Kilo Gateway's unified API to access many models in PASO"
title: "Kilo Gateway"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via Kilo Gateway in PASO
---
...
PASO does not publish a task-to-upstream-model mapping for it; routing behind
`kilo-auto/balanced` is owned by Kilo Gateway.

At startup PASO queries `GET https://api.kilo.ai/api/gateway/models` and merges discovered models
...
<AccordionGroup>
  <Accordion title="Transport and compatibility">
    ...
    - Gemini-backed Kilo refs stay on the proxy-Gemini path: PASO sanitizes Gemini thought
  ...
  <Accordion title="Troubleshooting">
    - If model discovery fails at startup, PASO falls back to the static catalog containing `kilocode/kilo-auto/balanced`.
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full PASO configuration reference.
  </Card>
