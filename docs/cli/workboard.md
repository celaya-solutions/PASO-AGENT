...
Local `list`, `create`, `show`, and `move` commands operate on the local PASO state directory used by the current profile. Use `--dev` or `--profile <name>` on the top-level `openclaw` command when you need a different state root.
