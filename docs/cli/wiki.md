...
Rebuild indexes, related blocks, dashboards, and the compiled query/prompt snapshot. The snapshot is persisted in PASO's shared SQLite plugin state and kept in memory for synchronous prompt projection; it does not create cache files in the vault.
