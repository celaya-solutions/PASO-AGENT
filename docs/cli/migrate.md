...
read_when:
  - You want to migrate from Hermes or another agent system into PASO
...
<ParamField path="--no-backup" type="boolean">
  Skip the pre-apply backup. Requires `--force` when local PASO state exists.
</ParamField>
...
<AccordionGroup>
  ...
  <Accordion title="Backups">
    Apply creates and verifies a PASO backup before applying the migration. If no local PASO state exists yet, the backup step is skipped and the migration continues. To skip a backup when state exists, pass both `--no-backup` and `--force`.
  </Accordion>
...
- Project `CLAUDE.md` and `.claude/CLAUDE.md` into the PASO agent workspace (`AGENTS.md`).
...
- Claude command Markdown files (user `~/.claude/commands` and project `.claude/commands`) converted into PASO skills with manual invocation only.

### Archive and manual-review state

Claude hooks, permissions, environment defaults, project `CLAUDE.local.md`, `.claude/rules`, user and project `agents/` directories, and project history (`projects`, `cache`, `plans` under `~/.claude`) are preserved in the migration report or reported as manual-review items. PASO does not execute hooks, copy broad allowlists, or import OAuth/Desktop credential state automatically.
...
Use this provider when moving to the PASO Codex harness and you want to promote useful personal Codex CLI assets deliberately. Local Codex app-server launches use a per-agent `CODEX_HOME`, so they do not read your personal `~/.codex` by default. The normal process `HOME` is still inherited, so Codex can see shared `$HOME/.agents/*` skills/plugin marketplace entries and subprocesses can find user-home config and tokens.
...
import those credentials into the owning agent's PASO auth store explicitly.
...
- ChatGPT OAuth or OpenAI API-key credentials from `$CODEX_HOME/auth.json`,
  imported into the agent's PASO auth store only when `--include-secrets`
  is set.
...
- Personal AgentSkills under `$HOME/.agents/skills`, copied into the current PASO agent workspace for per-agent ownership.
...
- Memory config defaults for PASO file memory, plus archive or manual-review items for external memory providers such as Honcho.
...
- Current Hermes OpenAI Codex OAuth credentials and OpenCode OpenAI OAuth credentials when interactive credential migration is accepted, or when `--include-secrets` is set. Do not keep Hermes and PASO using the same imported refresh grant.
...
Hermes state that PASO cannot safely interpret is copied into the migration report for manual review, but it is not loaded into live PASO config or credentials. This includes `plugins/`, `sessions/`, `logs/`, `cron/`, `mcp-tokens/`, `plans/`, `workspace/`, `skins/`, `kanban/`, pairing/platform state, gateway routing/process state, and the detected Hermes SQLite databases.
...
<Note>
Onboarding imports require a fresh PASO setup. Reset config, credentials, sessions, and the workspace first if you already have local state. Backup-plus-overwrite or merge imports are feature-gated for existing setups.
</Note>
...
- [Migrating](/install/migrating): move PASO to a new machine.
