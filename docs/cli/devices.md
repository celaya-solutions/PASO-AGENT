...
<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), PASO supersedes the previous pending entry with a new `requestId`. Run `openclaw devices list` right before approval to get the current id.
</Note>

Approval behavior:

- If the device is already paired and requests broader scopes or role, PASO keeps the existing approval and creates a new pending upgrade request. Compare `Requested` vs `Approved` in `openclaw devices list`, or preview with `--latest`, before approving.
