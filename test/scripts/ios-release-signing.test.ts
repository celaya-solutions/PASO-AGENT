...
function runSigning(mode: string, manifestPath?: string): string {
  return execFileSync(
    process.execPath,
    [SCRIPT, "--mode", mode, ...(manifestPath ? ["--manifest", manifestPath] : [])],
    {
      encoding: "utf8",
      stdio: ["ignore", "pipe", "pipe"],
    },
...
function withEnabledManifest<T>(callback: (manifestPath: string) => T): T {
  const directory = mkdtempSync(path.join(tmpdir(), "paso-ios-signing-test-"));
  const manifestPath = path.join(directory, "AppStoreSigning.json");
  const source = JSON.parse(
    readFileSync(path.join(process.cwd(), "apps/ios/Config/AppStoreSigning.json"), "utf8"),
  ) as Record<string, unknown>;
  source.releaseEnabled = true;
...
describe("scripts/ios-release-signing.mjs", () => {
  ...
  it("emits manual App Store profile settings for every signed target", () => {
    const output = withEnabledManifest((manifestPath) => runSigning("xcconfig", manifestPath));
  ...
  it("documents the canonical release signing plan", () => {
    ...
    expect(output).toContain("Release enabled: no");
    expect(output).toContain("Release owner: upstream-openclaw-compatibility");
  ...
  it("blocks the inherited signing manifest from publishing PASO", () => {
    const result = runSigningResult(["--mode", "xcconfig"]);

    expect(result.ok).toBe(false);
    expect(result.stderr).toContain("PASO iOS publishing is disabled");
  });
});
