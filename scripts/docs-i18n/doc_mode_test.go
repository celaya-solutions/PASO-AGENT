...
func (docFrontmatterTranslator) Translate(_ context.Context, text, _, _ string) (string, error) {
	replacer := strings.NewReplacer(
		"Step-by-step Fly.io deployment for PASO with persistent storage and HTTPS", "在 Fly.io 上逐步部署 PASO，包含持久化存储和 HTTPS",
		"Deploying PASO on Fly.io", "在 Fly.io 上部署 PASO",
...
func (docFrontmatterFallbackTranslator) Translate(_ context.Context, text, _, _ string) (string, error) {
	switch text {
	case "Step-by-step Fly.io deployment for PASO with persistent storage and HTTPS":
		return strings.Join([]string{
			"<frontmatter>",
			"title: Fly.io",
			"summary: \"在 Fly.io 上部署 PASO 的逐步指南，包含持久化存储和 HTTPS 设置\"",
			"read_when:",
			"  - 在 Fly.io 上部署 PASO",
			"  - 设置 Fly 卷、机密和初始运行配置",
			"</frontmatter>",
	...
	case "Deploying PASO on Fly.io":
		return "在 Fly.io 上部署 PASO", nil
...
func TestProcessFileDocUsesFieldLevelFrontmatterTranslation(t *testing.T) {
	...
	text := string(output)
	if !strings.Contains(text, "在 Fly.io 上逐步部署 PASO，包含持久化存储和 HTTPS") {
		t.Fatalf("expected translated summary in output:\n%s", text)
	}
	if !strings.Contains(text, "在 Fly.io 上部署 PASO") {
...
func TestProcessFileDocRejectsSuspiciousFrontmatterScalarExpansion(t *testing.T) {
	...
	if !strings.Contains(text, "summary: Step-by-step Fly.io deployment for PASO with persistent storage and HTTPS") {
...
func TestValidateDocChunkTranslationRejectsMovedProtectedProductLinkLabel(t *testing.T) {
	...
	source := "Deploy PASO on [Render](https://render.com) using the Blueprint.\n"
	translated := "Render पर Blueprint का उपयोग करके [PASO](https://render.com) परिनियोजित करें।\n"
...
func TestValidateDocBodyRejectsMovedProtectedReferenceLinkLabel(t *testing.T) {
	...
	translated := "Auf Render mit [PASO][provider] bereitstellen.\n\n[provider]: https://render.com\n"
	err := validateDocBodyFencedLiterals(source, translated)
	if err == nil || !strings.Contains(err.Error(), "protected link label mismatch") {
