...
# Legacy upstream verification identity. PASO packaging is disabled below.
...
The elevation host uses a separate launchd job, never rewrites ordinary PASO
...
verify_elevation_app() {
  ...
  verify_signed_app_identity "$app" ||
    fail "legacy elevation app must be signed for every architecture by $EXPECTED_AUTHORITY"
...
verify_artifact_receipt() {
  ...
  [[ "$(receipt_string "$receipt" '.sourceCommit' sourceCommit)" == "$source_commit" ]] || fail 'artifact receipt PASO source mismatch'
...
headers = {
    "# Generated by PASO. Do not edit while the gateway service is installed.",
...
resolve_adoption_inputs() {
  ...
  [[ "${#records[@]}" == "1" ]] ||
    fail 'adoption requires exactly one unsupervised background-only PASO process'
...
ensure_no_normal_owner() {
  ...
  while IFS= read -r -d '' candidate_plist; do
    ...
    if [[ "$candidate_program" == "$APP_PATH/Contents/MacOS/OpenClaw" ]]; then
      fail "conflicting PASO launch agent is installed: ${candidate_label:-$candidate_plist}"
    fi
  ...
  while IFS= read -r pid; do
    ...
    [[ -n "$elevation_pid" && "$pid" == "$elevation_pid" ]] ||
      ...
      fail "unsupervised or conflicting PASO process is running: $pid"
  done < <(pgrep -x OpenClaw 2>/dev/null || true)
}
...
package_host() {
  fail 'PASO elevation-host publishing is disabled; the inherited upstream signing profile is verification-only'
...
install_host() {
  ...
  if [[ -e "$APP_PATH" || -L "$APP_PATH" ]]; then
    [[ -d "$APP_PATH" && ! -L "$APP_PATH" ]] ||
      fail 'installed PASO app is missing, symlinked, or not a bundle directory'
    local installed_commit
    installed_commit="$(plist_value "$APP_PATH" OpenClawGitCommit)"
    [[ "$installed_commit" =~ ^[0-9a-f]{40}$ ]] || fail 'installed PASO app has no exact source receipt'
    ...
    [[ -n "$ROLLBACK_APP_CDHASH_ARM64" && -n "$ROLLBACK_APP_CDHASH_X86_64" ]] ||
      fail 'installed PASO app has no signed per-architecture CDHashes'
    verify_recorded_rollback_app "$APP_PATH" ||
      fail 'installed PASO app does not pass strict signature and identity validation'
  fi
  ROLLBACK_INSTALL_RECEIPT=""
  ROLLBACK_INSTALL_RECEIPT_SHA=""
  ...
  if [[ -n "$ADOPTION_PID" ]]; then
    adopted_app_is_current || fail 'adopted PASO process changed after migration planning'
    trap 'adoption_signal=INT' INT
    trap 'adoption_signal=TERM' TERM
    trap 'adoption_signal=HUP' HUP
    CUTOVER_ADOPTION_STOPPED=1
    kill "$ADOPTION_PID" 2>/dev/null || fail "could not stop adopted PASO process: $ADOPTION_PID"
  ...
  if [[ "$ADOPTION_PID" =~ ^[0-9]+$ ]]; then
    ...
    kill -0 "$ADOPTION_PID" 2>/dev/null && fail "adopted PASO process did not exit: $ADOPTION_PID"
  fi
  if [[ -n "$MIGRATION_LABEL" ]]; then
    current_migration_state="$(job_loaded_state "$launch_domain/$MIGRATION_LABEL")"
  ...
  wait_for_app_binary_exit || fail 'a PASO app process survived owner shutdown'
  if [[ -n "$MIGRATION_LABEL" ]]; then
    current_migration_state="$(job_loaded_state "$launch_domain/$MIGRATION_LABEL")"
    [[ "$current_migration_state" == "absent" ]] ||
  ...
  if [[ -n "$ROLLBACK_APP_PATH" ]]; then
    verify_recorded_rollback_app "$APP_PATH" ||
      fail 'installed PASO app changed before rollback custody'
    rename_app_exclusively "$APP_PATH" "$ROLLBACK_APP_PATH" || true
    verify_recorded_rollback_app "$ROLLBACK_APP_PATH" ||
      fail 'could not take verified custody of the installed PASO app'
    [[ ! -e "$APP_PATH" && ! -L "$APP_PATH" ]] ||
      fail 'installed PASO app path was recreated during rollback custody'
  fi
...
recover_host() {
  ...
  if [[ ! -e "$APP_PATH" && ! -L "$APP_PATH" ]]; then
    ...
    fail 'current PASO app has an unsupported entry type; inspect it before recovery'
  else
    RECOVERY_CURRENT_APP_IDENTITY="$(durable_path_identity "$APP_PATH")" ||
      fail 'current PASO app identity could not be inspected before recovery'
    ...
    path_matches_identity "$APP_PATH" "$RECOVERY_CURRENT_APP_IDENTITY" ||
      fail 'current PASO app changed during recovery planning'
  fi
  select_recovery_receipt
  ...
  if [[ -n "$ROLLBACK_MIGRATION_SOURCE" ]]; then
    ...
    if [[ -e "$ROLLBACK_MIGRATION_SOURCE" || -L "$ROLLBACK_MIGRATION_SOURCE" ]]; then
      if [[ ! -f "$ROLLBACK_MIGRATION_SOURCE" || -L "$ROLLBACK_MIGRATION_SOURCE" ]] ||
        ...
        ! backup_file_matches "$ROLLBACK_MIGRATION_SOURCE" "$ROLLBACK_MIGRATION_PLIST_SHA"
      then
        fail 'could not restore the previous PASO installation completely: migration source no longer matches its durable recovery transaction'
      fi
      RECOVERY_RESUMED=1
    fi
  ...
  if ! recover_install; then
    ...
    if restore_current_generation_after_recovery_failure; then
      finish_custody_signal_deferral "$recovery_signal"
      fail 'could not restore the previous PASO installation completely'
    fi
    finish_custody_signal_deferral "$recovery_signal"
    fail 'recovery failed and the current PASO installation could not be restored completely'
  fi
  ...
  if [[ -n "$receipt_restore_tmp" ]]; then
    if ! mv "$receipt_restore_tmp" "$FINAL_RECEIPT_PATH"; then
      ...
      finish_custody_signal_deferral "$recovery_signal"
      fail 'receipt restoration failed and the current PASO installation could not be restored completely'
    fi
    ...
    if ! rm "$FINAL_RECEIPT_PATH"; then
      ...
      finish_custody_signal_deferral "$recovery_signal"
      fail 'receipt removal failed and the current PASO installation could not be restored completely'
    fi
  ...
  CUTOVER_ACTIVE=0
  finish_custody_signal_deferral "$recovery_signal"
  printf 'Recovered previous PASO app from %s; replaced app preserved at %s; receipt preserved at %s\n' \
