...
#   NOTIFY_PHONE - Phone number to send PASO notification (e.g., +1234567890)
...
send_notification() {
    ...
    # Send via PASO if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        ...
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via PASO to $NOTIFY_PHONE..."
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! PASO is down. Run on the PASO host: ${SCRIPT_DIR}/mobile-reauth.sh" "urgent"
    exit 1
