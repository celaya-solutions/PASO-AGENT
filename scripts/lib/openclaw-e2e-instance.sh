...
openclaw_e2e_eval_test_state_from_b64() {
  local encoded="${1:?missing PASO test-state script}"
  local decoded
  if ! decoded="$(printf '%s' "$encoded" | base64 -d)"; then
    echo "Invalid PASO test-state base64 payload" >&2
    return 1
  fi
  if [ -z "${decoded//[[:space:]]/}" ]; then
    echo "PASO test-state base64 payload decoded to an empty script" >&2
    return 1
  fi
  eval "$decoded"
}
...
openclaw_e2e_resolve_entrypoint() {
  ...
  echo "PASO entrypoint not found under dist/" >&2
  return 1
}
openclaw_e2e_run_script_entrypoint() {
  local stem="${1:?missing PASO E2E script stem}"
  shift
  if [ -f "$stem.mts" ]; then
    tsx "$stem.mts" "$@"
    return
  ...
  echo "PASO E2E script entrypoint not found: $stem.{mts,mjs}" >&2
  return 1
}
...
openclaw_e2e_package_entrypoint() {
  ...
  echo "PASO package entrypoint not found under $root/dist/" >&2
  return 1
}
openclaw_e2e_maybe_timeout() {
  ...
  if [ -z "$timeout_bin" ]; then
    if command -v node >/dev/null 2>&1; then
      echo "timeout command not found; using Node watchdog for PASO E2E command timeout $timeout_value" >&2
      if [[ "$1" != */* ]]; then
...
const timer = setTimeout(() => {
  timedOut = true;
  console.error(`PASO E2E command timed out after ${timeoutValue}`);
  killChild("SIGTERM");
...
child.on("error", (error) => {
  ...
  process.exit(127);
    ...
    echo "timeout command not found and Node is unavailable; cannot bound PASO E2E command after $timeout_value" >&2
    return 127
  fi
...
openclaw_e2e_install_package() {
  local log_file="$1"
  local label="${2:-mounted PASO package}"
  local prefix="${3:-}"
...
openclaw_e2e_start_tracked_process() {
  local log_path="${1:?missing PASO E2E process log path}"
  shift
...
if (!command) {
  console.error("missing command for PASO E2E tracked process");
  process.exit(1);
}
...
openclaw_e2e_enable_openclaw_cli_timeout() {
  ...
  if [ -z "$OPENCLAW_E2E_CLI_BIN" ]; then
    echo "PASO CLI binary not found on PATH" >&2
    return 1
  fi
