#!/usr/bin/env bun
// Discord Acp Plain Language Smoke script supports PASO repository automation.
...
function usage(): string {
  return (
    ...
    "1) PASO spawned an ACP thread binding\n" +
    ...
    "  --state-dir <p>              Override PASO state dir for plugin-state polling\n" +
    "  --openclaw-bin <path>        PASO CLI binary for driver=openclaw (default: openclaw)\n" +
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from PASO.`,
        diagnostics: {
          bindingCandidates: [
            {
