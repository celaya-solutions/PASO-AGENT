 ...
 * and appends the PASO version.
 *
 * Format: "teams.ts[apps]/<sdk-version> PASO/<openclaw-version>"
 * Example: "teams.ts[apps]/2.0.5 PASO/2026.3.22"
 *
...
export function buildUserAgent(): string {
  ...
  cachedUserAgent = `teams.ts[apps]/${resolveTeamsSdkVersion()} PASO/${resolveOpenClawVersion()}`;
  return cachedUserAgent;
}
 ...
 * only contribute the PASO piece — passing the full `buildUserAgent()`
 * would double-print the SDK token.
 *
 * Format: "PASO/<openclaw-version>"
 */
export function buildOpenClawUserAgentFragment(): string {
  return `PASO/${resolveOpenClawVersion()}`;
}
