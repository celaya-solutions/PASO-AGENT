...
export function meetStatusScript(params: {
  ...
  const audioDeviceFamily = (value) => {
    ...
    if (/\\b(?:PASO|OpenClaw) Meeting Audio\\b/i.test(label)) return 'openclaw-meeting-audio';
  ...
  const routeMeetAudioInput = async () => {
    ...
    try {
      ...
      audioInputDeviceLabel = input.label || 'PASO meeting audio';
  ...
  const routeMeetAudioOutput = async () => {
    ...
    try {
      ...
      audioOutputDeviceLabel = output.label || "PASO meeting audio";
  ...
  const captionState = (() => {
    ...
    // A reused tab starts a fresh logical transcript for each PASO session.
  ...
  if (!inCall && (host === "accounts.google.com" || /use your google account|to continue to google meet|choose an account|sign in to (join|continue)/i.test(pageText))) {
    manualAction = manualActionFor("google-login-required", "Sign in to Google in the PASO browser profile, then retry the Meet join.");
    ...
    manualAction = manualActionFor("meet-admission-required", "Admit the PASO browser participant in Google Meet, then retry speech.");
  } else if (permissionNeeded) {
    manualAction = manualActionFor("meet-permission-required", allowMicrophone ? "Allow microphone/camera/speaker permissions for Meet in the PASO browser profile, then retry." : "Join without microphone/camera permissions in the PASO browser profile, then retry.");
    ...
    manualAction = manualActionFor(
      ...
      "Select BlackHole 2ch or PASO Meeting Audio as both the Meet microphone and speaker, then retry."
    );
    ...
    manualAction = manualActionFor("meet-audio-choice-required", allowMicrophone ? "Meet is showing the microphone choice. Click Use microphone in the PASO browser profile, then retry." : "Meet is showing the microphone choice. Choose the no-microphone option in the PASO browser profile, then retry.");
  }
