# GitHub Copilot agent runtime (PASO plugin)

External PASO plugin that registers a `copilot` agent harness backed by `@github/copilot-sdk` and the GitHub Copilot CLI.
...
entry; `auto` never picks it. PASO remains the default embedded runtime.
