...
function spawnRaftBridge(params: {
  ...
  // Raft owns the fixed bridge command. PASO passes profile/loopback
...
export async function startRaftGatewayAccount(
  ...
  if (!ctx.channelRuntime) {
    throw new Error("Raft requires PASO channel runtime support. Update PASO and retry.");
  }
  ...
  const server = createServer((request, response) => {
    void (async () => {
      ...
      if (
        ...
        // Raft drains runtime activity after each wake pass. PASO has no
      ...
      // Raft owns wake metadata and its schema evolution. PASO accepts only
