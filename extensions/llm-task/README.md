...
`openclaw.invoke --each`) without adding new PASO code per workflow.
...
This extension depends on PASO internal modules (the embedded agent runner).
It is intended to ship as a **bundled** PASO extension (like `lobster`) and
