...
describe("resolveTelegramFetch", () => {
  ...
  it("uses the PASO debug proxy URL when no explicit proxy fetch is provided", async () => {
  ...
  it("uses OPENCLAW_PROXY_URL as a Telegram explicit proxy when proxy env is absent", async () => {
    ...
    expect(ProxyAgentCtor).toHaveBeenCalledTimes(1);
    const proxyOptions = constructorOptions(ProxyAgentCtor, "PASO proxy") as {
      allowH2?: boolean;
      uri?: string;
  ...
  it("keeps resolver-scoped transport policy for PASO proxy fetches", async () => {
