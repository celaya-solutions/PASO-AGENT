...
function formatLegacyClawdBrowserProfileResidueNote(
  ...
  return [
    ...
    `- The canonical PASO-managed browser profile is ${residue.canonicalUserDataDir}.`,
...
export async function noteChromeMcpBrowserReadiness(
      ...
      if (extension.manualSetupRequired) {
        noteFn(
          [
            ...
            `- Run ${formatCliCommand("openclaw browser extension install")} before adding PASO from ${FOUNDATION_CHROME_WEB_STORE_URL}.`,
  ...
  if (!browserExecutable && managedProfiles.length > 0) {
    noteFn(
      [
        `- PASO-managed browser profile(s) are configured: ${managedProfileLabel}.`,
        "- No Chromium-based browser executable was found on this host for PASO-managed launch.",
  ...
  if (missingDisplay || shouldWarnRootNoSandbox) {
    const lines = [`- PASO-managed browser profile(s) are configured: ${managedProfileLabel}.`];
    if (missingDisplay) {
      lines.push(
  ...
  if (autoConnectProfiles.length === 0) {
    noteFn(
      [
        ...
        "- Keep the browser running and accept the attach consent prompt the first time PASO connects.",
      ].join("\n"),
      "Browser",
    );
    return;
  ...
  if (!chrome) {
    const lines = [
      `- Chrome MCP existing-session is configured for profile(s): ${profileLabel}.`,
      `- Google Chrome was not found on this host for auto-connect profile(s): ${autoProfileLabel}. PASO does not bundle Chrome.`,
