...
/** Retired PASO auth profile replaced by Claude CLI's native login. */
...
/** Non-secret marker telling PASO that the installed Claude CLI owns auth. */
...
// steer PASO-managed Claude CLI runs toward a different provider,
...
/** Environment variables removed before launching PASO-managed Claude CLI runs. */
export const CLAUDE_CLI_CLEAR_ENV = [
  ...
  // Re-injected per run from PASO's canonical context budget.
  ...
  // Re-injected per run from PASO's effective thinking level.
  "MAX_THINKING_TOKENS",
