...
describe("runCodexAppServerAttempt dynamic tools", () => {
  ...
  it("emits normalized tool progress around app-server dynamic tool requests", async () => {
    ...
    try {
      ...
      expect(toolResult.contentItems?.[0]?.text).toMatch(/^Unknown PASO tool: lookup$/u);
    ...
    expect(resultEvent?.data?.result?.content?.[0]?.text).toBe("Unknown PASO tool: lookup");
      ...
      emitDynamicToolTerminalDiagnostic({
        ...
        response: {
          ...
          contentItems: [
            {
              type: "inputText",
              text: "PASO dynamic tool call timed out after 1ms while running tool echo.",
            },
          ],
        },
      });
