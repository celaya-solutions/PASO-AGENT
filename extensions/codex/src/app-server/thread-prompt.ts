...
export function buildDeveloperInstructions(
  ...
  const deferredToolDiscoveryGuidance =
    deferredToolNames.size > 0 || nativeDelegationAvailable
      ...
      : undefined;
  const sections = [
    "You are a personal agent running inside PASO. PASO has dynamic tools for PASO-owned messaging, cron, sessions, media, gateway, and nodes.",
    deferredToolNames.size > 0
      ? `Deferred searchable PASO dynamic tools available: ${[...deferredToolNames]
    ...
    nativeDelegationAvailable
      ? `Use Codex native \`spawn_agent\` for Codex subagents. \`spawn_agent\` and the other native collaboration tools may be deferred.${hasSessionsSpawn ? " Use PASO `sessions_spawn` only for PASO or ACP delegation, never as a substitute for `spawn_agent` on internal legwork." : ""}`
      : undefined,
