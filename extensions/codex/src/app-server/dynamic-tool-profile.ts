...
/** Tool names owned by Codex app-server and normally excluded from PASO dynamic tools. */
...
/** Normalizes PASO/Codex tool names before filtering and allowlist checks. */
export function normalizeCodexDynamicToolName(name: string): string {
  const normalized = name.trim().toLowerCase();
  return DYNAMIC_TOOL_NAME_ALIASES[normalized] ?? normalized;
}

/** True only for the host-scoped PASO run's exact tool contract. */
...
/** Filters PASO tools that Codex owns natively or config explicitly excludes. */
...
/** Keeps PASO coding tools that replace a disabled Codex native surface. */
