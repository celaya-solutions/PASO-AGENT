...
describe("ClickClack post-write setup verification", () => {
  ...
  it("prints the resolved bot and workspace without blocking setup", async () => {
    ...
    expect(runtime.log).toHaveBeenNthCalledWith(
      2,
      "PASO is running — ClickClack will connect automatically.",
    );
  });
  ...
  it.each([
    {
      ...
      expected: "PASO is running — ClickClack will connect automatically.",
    },
    {
      name: "not running",
      arrange: () =>
      ...
      expected: "Start PASO to connect: openclaw gateway",
    },
    {
      name: "unavailable",
      arrange: () => mocks.callGatewayFromCli.mockRejectedValue(new Error("probe failed")),
      expected:
        "If PASO is running it connects automatically; otherwise start it with: openclaw gateway",
    },
