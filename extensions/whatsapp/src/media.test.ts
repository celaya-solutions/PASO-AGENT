...
describe("web media loading", () => {
  beforeAll(() => {
    ...
    // Also keep it outside the PASO temp root so default localRoots doesn't accidentally make all state readable.
...
describe("local media root guard", () => {
  ...
  it("allows default PASO state workspace and sandbox roots", async () => {
  ...
  it("rejects default PASO state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
