...
Official MXC sandbox execution plugin for PASO.

This plugin lets PASO run tool execution through MXC on Windows hosts with
...
- `workspaceAccess`:
  ...
  - `rw`: the active agent workspace is mounted read-write. If protected
    PASO skill roots (`skills`, `.agents/skills`, or the materialized
...
- `scope` workspace selection:
  - `session`, `agent`, and `shared` choose the PASO workspace directory
    passed to MXC.
...
- PASO passes per-run command, environment, and filesystem config to the
...
Use an already configured PASO installation with MXC installed and enabled
...
[`openclaw config patch --stdin`](https://github.com/celaya-solutions/PASO-AGENT/blob/main/docs/cli/config.md#config-patch)
...
```json
{
  "filesystem": {
    ...
    "additionalReadonlyPaths": ["C:\\Tools\\PASO\\shared-readonly"],
...
When multiple configured policy files exist, PASO layers them
...
The filesystem bridge keeps protected PASO skill overlays read-only. For
