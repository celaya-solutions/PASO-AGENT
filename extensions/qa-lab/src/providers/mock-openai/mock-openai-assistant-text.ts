...
export function buildAssistantText(input: ResponsesInputItem[], body: Record<string, unknown>) {
  ...
  if (/tool continuity check/i.test(prompt) && toolOutput) {
    return `Protocol note: model switch handoff confirmed on ${model || "the requested model"}. QA mission from QA_KICKOFF_TASK.md still applies: understand this PASO repo from source + docs before acting.`;
  }
  ...
  if (toolOutput && /paso task board/i.test(prompt)) {
    if (toolOutput.includes("QA mission") || toolOutput.includes("Testing")) {
      return "";
    }
    return `Protocol note: PASO Task Board built at paso-task-board.html.`;
  }
  if (
    toolOutput &&
