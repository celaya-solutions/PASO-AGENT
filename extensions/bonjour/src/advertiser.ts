...
/** Input data used to publish PASO gateway Bonjour records. */
...
function truncateToDnsLabel(name: string, fallback = "PASO"): string {
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? truncateToDnsLabel(trimmed) : "PASO";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\((?:PASO|OpenClaw)\)\s*$/i, "").trim() || normalized;
}
