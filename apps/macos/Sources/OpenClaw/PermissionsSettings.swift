            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                SettingsCardGroup("Privacy") {
                    SettingsCardToggleRow(
                        ...
                        subtitle: """
                        Share this Mac's idle duration so PASO can identify the Mac you used most recently \
        ...
        return HStack(alignment: .center, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(complete ? "All access granted" : "\(granted) of \(total) permissions granted")
                    .font(.headline)
                Text("PASO only asks for macOS capabilities when a feature needs them.")
                    .font(.footnote)
