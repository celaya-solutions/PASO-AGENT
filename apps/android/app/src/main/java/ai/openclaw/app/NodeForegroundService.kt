          ...
          val title =
            when {
              state.connected && state.mode == VoiceCaptureMode.TalkMode ->
                nativeString("PASO Node · Talk")
              state.connected -> nativeString("PASO Node · Connected")
              else -> nativeString("PASO Node")
            }
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = nativeString("PASO node connection status")
        setShowBadge(false)
      }
