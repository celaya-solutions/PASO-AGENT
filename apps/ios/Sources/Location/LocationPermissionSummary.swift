...
struct LocationPermissionSummary: Equatable {
    ...
    var detailText: String {
        ...
        {
        case (.off, false, _, _):
            "Location sharing is disabled in PASO. Location Services are off in iOS Settings."
        case (.off, true, .authorizedAlways, _):
            "Location sharing is disabled in PASO. iOS currently allows Always."
        case (.off, true, .authorizedWhenInUse, _):
            "Location sharing is disabled in PASO. iOS currently allows While Using."
        case (.off, _, _, _):
        ...
        @unknown default:
            "PASO cannot determine the current iOS location permission."
        }
    }
}
