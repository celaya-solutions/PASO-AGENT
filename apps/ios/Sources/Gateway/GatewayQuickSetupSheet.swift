...
private struct GatewayQuickSetupHeader: View {
    ...
    private var subtitle: LocalizedStringKey {
        if self.hasCandidate {
            return "PASO found a gateway on this network. Review it, then pair this iPhone as a secure node."
        }
        return "PASO is searching the local network and tailnet for a Gateway you can trust."
    }
}
