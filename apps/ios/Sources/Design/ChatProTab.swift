...
struct ChatProTab: View {
    ...
    private var content: some View {
        self.chatSurface
                ...
                isPresented: self.$showsTranscriptExportError)
            {
                ...
                Text("PASO could not prepare the Markdown file.")
                    .font(OpenClawType.body)
            }
    }
    ...
    nonisolated static let emptyAssistantPrompts: [OpenClawChatView.StarterPrompt] = [
        OpenClawChatView.StarterPrompt(
            id: "summarize-status",
            title: String(localized: "Check PASO status"),
            prompt: String(localized: "Summarize the current PASO status and tell me what needs attention.")),
        OpenClawChatView.StarterPrompt(
            id: "show-controls",
