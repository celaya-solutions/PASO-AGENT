...
final class IOSSystemAgentChatModel {
        ...
        routeIdentity: String?)
    {
        ...
        self.errorMessage = String(localized: "The Gateway connection changed. Restart PASO to reconnect.")
    }
...
struct SettingsSystemAgentChatScreen: View {
    ...
    private var accessGateDetail: String {
        switch self.model.accessState {
        case .disconnected:
            String(localized: "Connect this iPhone to a Gateway before opening the PASO settings assistant.")
        case .missingAdminScope:
            String(localized: "Reconnect with operator.admin access to review and change Gateway settings.")
        case .checkingSystemAgentMethod:
            String(localized: "Checking whether this Gateway supports the PASO settings assistant.")
        case .missingSystemAgentMethod:
            String(localized: "Update this Gateway to use the PASO settings assistant.")
        case .ready:
            ""
        }
    }
    ...
    private var handoffRow: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("PASO is ready to continue in your ordinary chat.")
