...
Use these steps to exercise the live PASO iOS App Review Gateway.

## Demo Account / Setup

Use the PASO iOS app with the live review Gateway setup code included in
...
9. On `Connected`, tap `Open PASO`.
10. Confirm the `Control` screen shows `Gateway Online`.
11. Tap `Settings`.
12. Tap `Approvals`.
13. Tap `Open Notifications`.
14. Tap `Enable Notifications`.
15. On `Enable PASO Hosted Push Relay?`, tap `Continue`.
16. If iOS asks whether PASO may send notifications, tap `Allow`.
...
command `printf 'PASO App Review approval demo complete\n'`. Tap
...
9. While Talk is active, send PASO to the background by returning to the
   Home Screen or locking the iPhone. Do not force quit the app.
10. Continue speaking then wait for assistant audio reply.

Expected result: realtime Talk audio continues while PASO is backgrounded.
Reopen PASO, confirm Talk is still active, then tap `Stop Talk`.
...
1. Tap `Settings`.
2. Tap `Reconnect`.
3. Immediately send PASO to the background by returning to the Home Screen
   or locking the iPhone.
4. Watch the Lock Screen or Dynamic Island while the Gateway reconnects.

Expected result: while reconnecting, iOS can show a `PASO` Live Activity
with connection status such as `Connecting...` or `Reconnecting...`. On a fast
network this status may be brief because PASO ends the Live Activity after
...
4. Immediately send PASO to the background and lock the iPhone. Do not
   force quit the app.

Expected result: the iPhone Lock Screen receives a visible `PASO`
notification with this body:

```text
PASO App Review push notification demo
```

Tap the notification and unlock the iPhone if prompted. If PASO opens on
...
3. Immediately send PASO to the background and lock the iPhone. Do not
   force quit the app.
4. Wait for the `PASO` notification on the Lock Screen. It normally appears
   about 10 seconds after the message is sent.
5. Tap the notification and unlock the iPhone if prompted. If PASO opens on
   `Control`, tap `Chat`.
...
PASO does not present an aggregate permissions page during onboarding.
...
the native iOS authorization prompt, with no additional PASO dialog or
deferral action.
...
permission sheet. PASO requests read-only access to steps, sleep, resting
...
4. Tap `Share`.
5. Tap `PASO`.
6. Confirm the PASO share extension appears and shows
   `Edit text, then tap Send.` and `Send to PASO`.
7. Tap `Send to PASO`.

Expected result: the PASO share extension sends the shared Safari page to
the live review Gateway and shows `Sent to PASO.` Returning to PASO
