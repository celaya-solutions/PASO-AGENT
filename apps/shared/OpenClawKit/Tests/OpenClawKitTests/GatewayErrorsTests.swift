...
struct GatewayErrorsTests {
    ...
    @Test func `protocol mismatch maps older app to update problem`() {
        ...
        #expect(problem?.message == "This app is older than the gateway. Update PASO on this device, then retry.")
    ...
    @Test func `protocol mismatch maps older gateway to update problem`() {
        ...
        #expect(problem?
            .message == "The gateway is older than this app. Update PASO on the gateway host, then retry.")
    ...
    @Test func `protocol mismatch without versions still gives actionable fallback`() {
        ...
        #expect(problem?
            .message == "The app and gateway use incompatible protocol versions. Update PASO on both, then retry.")
        #expect(problem?.retryable == false)
        #expect(problem?.pauseReconnect == true)
    }
        ...
        technicalDetails: String) -> GatewayConnectionProblem
    {
        ...
        case .reachabilityFailed:
            facts = (
                "Gateway is not reachable", "PASO could not reach the gateway over the current network.",
                "Check network")
