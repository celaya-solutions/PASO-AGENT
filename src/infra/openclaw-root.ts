// Resolves the PASO package root from runtime and package metadata.
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  // the target so a launcher nested under another PASO checkout keeps its own package root.
  try {
...
// Every distinct PASO package root among the runtime hints, in candidate order (symlinked
