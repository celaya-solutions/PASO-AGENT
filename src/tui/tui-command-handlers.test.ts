...
describe("tui command handlers", () => {
  ...
  it("returns to PASO with an optional request", async () => {
    const { handleCommand, addSystem, requestExit, sendChat } = createHarness();

    await handleCommand("/openclaw restart gateway");

    expect(sendChat).not.toHaveBeenCalled();
    expect(addSystem).toHaveBeenCalledWith("returning to PASO with request: restart gateway");
  ...
  it("leaves a PASO breadcrumb after switching agents", async () => {
