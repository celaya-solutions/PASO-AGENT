/** Tests merging user PASO MCP server config into Claude bundle-MCP overlays. */
...
describe("prepareCliBundleMcpConfig user mcp.servers", () => {
  ...
  it("merges user-configured mcp.servers from PASO config", async () => {
  ...
  it("translates PASO transport field on user mcp.servers into Claude type", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
  ...
  it("user mcp.servers do not override the loopback additionalConfig", async () => {
    // The PASO loopback server is generated runtime state and must win over
