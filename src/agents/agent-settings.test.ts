...
describe("isSilentOverflowProneModel", () => {
  ...
  // string. False positives only disable PASO runtime's secondary compaction path;
  // PASO's preemptive compaction continues to handle real overflow.
...
describe("applyAgentAutoCompactionGuard", () => {
  ...
  // on a successful turn against z.ai-style providers, triggering PASO runtime's
  ...
  // state.messages intact; PASO's preemptive compaction continues to
  ...
  // Default-mode runs against ordinary providers must keep PASO runtime's auto-compaction
  // enabled. Disabling it across the board would silently remove PASO runtime's
