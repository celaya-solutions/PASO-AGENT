...
vi.mock("../model-suppression.js", () => {
  ...
  return {
    ...
    buildSuppressedBuiltInModelError: ({
      ...
      if (isUnsupportedXaiMultiAgentModel(provider, id)) {
        return "Unknown model: xai/grok-4.20-multi-agent-0309. PASO does not currently support xAI multi-agent models; choose another xAI model. See https://github.com/celaya-solutions/PASO-AGENT/tree/main/docs";
      }
      return undefined;
    },
  };
...
describe("resolveModel", () => {
  ...
  it("skips PASO auth and model discovery during dynamic model resolution", async () => {
  ...
  it("defaults missing model cost before handing models to PASO", async () => {
  ...
  it.each([
    ...
    expect(result.error).toBe(
      'Unknown model: openai-codex/gpt-5.4. "openai-codex" is a legacy provider ID. Run `openclaw doctor --fix` to migrate legacy model and provider config to the current OpenAI format. If the provider has no authenticated profile, run `openclaw models status` to check provider auth and re-authenticate if needed. See https://github.com/celaya-solutions/PASO-AGENT/tree/main/docs',
    );
  });
  ...
  it("does not build a configured fallback for unsupported xAI multi-agent models", async () => {
    ...
    expect(result.model).toBeUndefined();
    expect(result.error).toBe(
      "Unknown model: xai/grok-4.20-multi-agent-0309. PASO does not currently support xAI multi-agent models; choose another xAI model. See https://github.com/celaya-solutions/PASO-AGENT/tree/main/docs",
    );
  });
