...
describe("describeEmbeddedAgentStreamStrategy", () => {
  ...
  it("describes default Codex fallback as PASO native", () => {
    expect(
      describeEmbeddedAgentStreamStrategy({
        currentStreamFn: undefined,
        model: {
...
describe("resolveEmbeddedAgentStreamFn", () => {
  ...
  it("routes Codex responses fallbacks through PASO native transport", async () => {
    // Codex OAuth models use the PASO native transport, with prompt-cache
  ...
  it("routes PASO native OpenAI-compatible provider streams through boundary-aware transports", async () => {
  ...
  it("injects the resolved run api key into the PASO native Codex Responses fallback", async () => {
  ...
  it("falls back to authStorage when no resolved api key is available for PASO native fallback", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const authStorage = {
  ...
  it("forwards the run abort signal into the PASO native fallback when callers omit one", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
  ...
  it.each(["run", "caller"] as const)(
    "cancels the authenticated PASO native fallback when the %s signal aborts",
    async (signalOwner) => {
      const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
      const runController = new AbortController();
      const callerController = new AbortController();
  ...
  it("forwards the run signal on the sync PASO native fallback path without auth credentials", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const runSignal = new AbortController().signal;
    useNativeStreamFn(nativeStreamFn as never);
    const streamFn = resolveEmbeddedAgentStreamFn({
  ...
  it("strips cache boundary markers on the PASO native fallback path", async () => {
