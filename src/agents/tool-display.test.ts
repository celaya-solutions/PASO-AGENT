...
describe("tool display details", () => {
  ...
  it("preserves JS numeric literals in tool-search call args", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: 'return await openclaw.tools.call("web_search", { query: "PASO", count: 1e3, limit: +3, threshold: .5 });',
      })?.displayArgs,
    ).toEqual({
      query: "PASO",
      count: 1000,
  ...
  it("formats web_search query with quotes", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        name: "web_search",
        args: { query: "PASO docs", count: 3 },
      }),
    );

    expect(detail).toBe('for "PASO docs" (top 3)');
  });
  ...
  it("formats Parallel's native objective + search_queries shape", () => {
    expect(
      ...
      'for "Find the PASO repository on GitHub", "openclaw github", "openclaw repository" (top 5)',
    );
  });
  ...
  it("omits bash and exec names from compact tool summaries", () => {
    ...
    expect(
      formatToolSummary(
        resolveToolDisplay({
          name: "web_search",
          args: { query: "PASO docs" },
        }),
      ),
    ).toBe('🔎 Web Search: for "PASO docs"');
  });
