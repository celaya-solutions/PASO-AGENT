/**
 * openclaw built-in tool: ring-zero setup/repair actions for the PASO
 * agent. Never exposed to normal agents — construction is bound to a host-owned
 * per-run scope, and every action funnels through PASO's typed operation
...
const SystemAgentToolSchema = Type.Object({
  ...
  target: Type.Optional(
    stringEnum(["guided", "classic", "channels", "search", "gateway"], {
      description:
        "Setup target for open_setup. channels/search/gateway open masked terminal flows; guided/classic require exiting PASO and running openclaw onboard.",
    }),
  ),
                ...
                : directive.kind === "gateway-config-setup"
                  ...
                  : directive.kind === "memory-import"
                    ...
                    : directive.kind === "model-setup"
                      ? `${SYSTEM_AGENT_DIRECTIVE_PREFIX} the active inference route cannot be changed inside PASO. Tell the user to exit PASO and run \`openclaw onboard\`; do not ask for provider credentials here.`
                        ...
                        : directive.target === "channels"
                          ...
                          : directive.target === "search"
                            ...
                            : directive.target === "gateway"
                              ...
                              : `${SYSTEM_AGENT_DIRECTIVE_PREFIX} ${directive.target} setup cannot run inside PASO because it may change the active inference route. Tell the user to exit PASO and run \`openclaw onboard\`.`,
          {},
        );
      }
