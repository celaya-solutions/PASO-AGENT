...
describe("resolveSandboxedMediaSource", () => {
  ...
  it("rejects symlinked PASO tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked PASO tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
  ...
  it("rejects symlinked PASO tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOpenClawTmp(
