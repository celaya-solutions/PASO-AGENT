...
it("identifies harnesses that expose PASO tools", () => {
...
describe("runAgentHarnessAttempt", () => {
  ...
  it.each(["codex", "copilot"] as const)(
    "binds the host PASO tool to the %s SDK construction path without leaking authority",
    async (harnessId) => {
  ...
  it.each([
  ...
  ])("rejects $name allowlists for private PASO authority", async ({ toolsAllow }) => {
    ...
    await expect(runAgentHarnessAttempt(params)).rejects.toThrow(
      'PASO host authority requires toolsAllow: ["openclaw"]',
    );
    expect(pluginRunAttempt).not.toHaveBeenCalled();
    expect(isHostScopedAgentToolActive("openclaw")).toBe(false);
  });

  it("keeps the host PASO allowlist across global, agent, and sandbox deny-all policy", async () => {
  ...
  it("binds the same host PASO scope to the built-in PASO harness", async () => {
    let toolNames: string[] = [];
  ...
  it("falls back to the PASO harness in auto mode when no plugin harness matches", async () => {
  ...
  it("allows the selected PASO harness to satisfy context-engine pre-prompt assembly", async () => {
  ...
  it("surfaces an auto-selected plugin harness failure instead of replaying through PASO", async () => {
  ...
  it("surfaces a forced plugin harness failure instead of replaying through PASO", async () => {
    registerFailingCodexHarness();
  ...
  it("falls back to PASO when the implicit OpenAI Codex harness is unavailable", async () => {
    expect(resolveAgentHarnessPolicy({ provider: "openai", modelId: "gpt-5.4" })).toEqual({
      runtime: "codex",
      runtimeSource: "implicit",
    });
  ...
  it("honors explicit PASO runtime for OpenAI agent model runs", async () => {
    const result = await runAgentHarnessAttempt({
  ...
  it("honors provider wildcard PASO runtime policy for OpenAI agent model runs", async () => {
    registerSuccessfulCodexHarness();
  ...
  it("leaves PASO harness params unchanged for channel group sender deny-all policy", async () => {
    await runAgentHarnessAttempt({
      ...createAttemptParams(groupSenderDenyAllConfig()),
      sessionKey: "agent:main:telegram:group:test-deny-room",
      messageProvider: "telegram",
  ...
  it("does not let a strict agent model plugin runtime fall back to PASO", async () => {
    await expect(
      runAgentHarnessAttempt({
...
describe("selectAgentHarness", () => {
  ...
  it("honors session-level PASO pins when selecting a harness", () => {
    const supports = vi.fn(() => ({ supported: true as const, priority: 100 }));
    registerAgentHarness({
      id: "codex",
      label: "Codex",
  ...
  it("keeps request-scoped transport overrides on the implicit PASO runtime", () => {
    registerAgentHarness({
      id: "codex",
      label: "Codex",
      supports: () => ({ supported: true, priority: 100 }),
  ...
  it("honors explicit PASO runtime overrides when selecting a harness", async () => {
    registerSuccessfulCodexHarness();
  ...
  it("treats legacy PI runtime overrides as the built-in PASO harness", async () => {
    registerSuccessfulCodexHarness();
  ...
  it("selects PASO when the implicit OpenAI Codex harness is unavailable", () => {
    expect(selectAgentHarness({ provider: "openai", modelId: "gpt-5.4" }).id).toBe("openclaw");
  });

  it.each(["default", "auto"] as const)(
    "falls back from configured %s to PASO when implicit Codex is unavailable or unsupported",
    (runtime) => {
  ...
  it.each(["default", "auto"] as const)(
    "keeps a custom OpenAI route on implicit PASO with configured %s",
    (runtime) => {
      const supports = vi.fn(() => ({ supported: true as const, priority: 100 }));
      registerAgentHarness(
        {
  ...
  it("keeps an existing session PASO pin when provider policy forces a plugin harness", () => {
    registerFailingCodexHarness();
  ...
  it("ignores env-forced PASO for OpenAI default runtime selection", () => {
  ...
  it("keeps host auth on the built-in PASO compaction fallback", async () => {
    await expect(
      maybeCompactAgentHarnessSession(
  ...
  it("does not compact a selected plugin harness through PASO when the plugin has no compactor", async () => {
    registerFailingCodexHarness();
  ...
  it.each([
    ...
    "returns PASO for explicit CLI runtime alias $alias on $provider instead of throwing MissingAgentHarnessError",
