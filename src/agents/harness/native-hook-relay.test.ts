...
describe("native hook relay registry", () => {
  ...
  it("uses the Codex no-op output when no PASO hook decides", async () => {
    const relay = registerNativeHookRelay({
      provider: "codex",
      sessionId: "session-1",
      runId: "run-1",
  ...
  it("maps Codex PreToolUse to PASO before_tool_call and blocks before execution", async () => {
  ...
  it("normalizes Codex exec_command cmd input before running PASO policy", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
  ...
  it("normalizes Codex exec_command argv cmd input before running PASO policy", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
  ...
  it("maps Codex PostToolUse to PASO after_tool_call observation", async () => {
    const afterToolCall = vi.fn();
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "after_tool_call", handler: afterToolCall }]),
    );
  ...
  it("maps Codex MCP PreToolUse to PASO before_tool_call and can block", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
    ...
    const response = await invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_input: {
          entities: [{ name: "PASO", entityType: "project", observations: ["test"] }],
        },
      },
    });
  ...
  it("maps Codex MCP PostToolUse to PASO after_tool_call observation", async () => {
    const afterToolCall = vi.fn();
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "after_tool_call", handler: afterToolCall }]),
    );
  ...
  it("routes Codex MCP PermissionRequest payloads through PASO approval policy", async () => {
    const relay = registerNativeHookRelay({
      provider: "codex",
      agentId: "agent-1",
      sessionId: "session-1",
  ...
  it("defers PermissionRequest when PASO approval does not decide", async () => {
