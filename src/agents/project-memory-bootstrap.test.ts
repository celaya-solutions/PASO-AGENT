...
describe("project memory bootstrap", () => {
  ...
  const entries: MemorySearchResult[] = [
    {
      ...
      score: 0.8,
      snippet:
        "Use the release helper. <!-- trigger: release helper --> <!-- importance: 8 --> <!-- project: github.com/celaya-solutions/PASO-AGENT -->",
      source: "memory" as const,
      projectKey: "github.com/celaya-solutions/PASO-AGENT",
      importance: 8,
      provenance: {
  ...
  async function prepareEntries(
    ...
    activeProjectKeys: string[] = ["github.com/celaya-solutions/PASO-AGENT"],
  ): Promise<string[]> {
  ...
  it("uses the dedicated curated listing instead of a daily-note-crowded search", async () => {
    ...
    const rendered = (
      await prepareProjectMemoryBootstrap({
        cfg: {},
        agentId: "main",
        activeProjectKeys: ["github.com/celaya-solutions/PASO-AGENT"],
      })
    ).join("\n");
    expect(rendered).toContain("Use the release helper.");
  ...
  it("builds scoped write guidance without capturing global memory", () => {
    ...
    expect(instruction).toContain("<!-- project: github.com/celaya-solutions/PASO-AGENT -->");
