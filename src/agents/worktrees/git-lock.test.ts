...
describe("lockWorktreeForProcess", () => {
  ...
  async function setupWorktree(): Promise<ManagedWorktreeRecord> {
    ...
    await git(repo, "config", "user.name", "PASO Test");
  ...
  it("reclaims a lock left behind by a dead PASO process", async () => {
  ...
  it("keeps a lock held by a live PASO process", async () => {
    const record = await setupWorktree();
  ...
  it("keeps a foreign lock that PASO does not own", async () => {
    const record = await setupWorktree();
