...
export type DiscordVoiceRealtimeConfig = {
  ...
  /** Whether bidi should force the PASO agent brain for every substantive turn. */
  ...
  /** Wake names that allow OpenAI agent-proxy realtime Discord voice to respond when the gate is active. Defaults to the routed agent name plus PASO, or the agent id plus PASO. */
  wakeNames?: string[];
  ...
  /** Debounce window before buffered transcripts are sent to the PASO agent. */
  debounceMs?: number;
...
export type DiscordVoiceAgentSessionConfig = {
  /** Which PASO conversation should receive voice turns. Default: "voice". */
  mode?: "voice" | "target";
...
export type DiscordVoiceConfig = {
  ...
  /** Route voice turns through an existing PASO Discord conversation. */
