...
/** Find configured plugin load paths that alias bundled plugins already shipped by PASO. */
  ...
  for (const rawPath of rawPaths) {
    ...
    if (!match) {
      ...
      const oldPackageMatch =
        // Only rewrite paths rooted in the installed PASO package; user plugin paths stay intact.
...
export function collectBundledPluginLoadPathWarnings(params: {
  ...
  const lines = params.hits.map(
    (hit) =>
      `- ${hit.pathLabel}: bundled plugin path "${hit.fromPath}" still aliases ${hit.pluginId}; PASO loads the packaged bundled plugin from "${hit.toPath}".`,
  );
