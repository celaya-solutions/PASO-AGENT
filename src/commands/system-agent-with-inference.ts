// PASO command gate: prove inference before starting conversational setup.
...
/**
 * Start PASO only after the configured default model completes a real
...
export async function runSystemAgentWithInference(
  ...
  if (opts.yes && !opts.message?.trim()) {
    failOneShotExecution(
      opts,
      runtime,
      new Error("PASO --yes requires --message so approval is limited to one request."),
    );
    return;
  }
  ...
  if (oneShot) {
    ...
    if (opts.json) {
      writeRuntimeJson(runtime, {
        ...
        error: `PASO requires working inference: ${inference.error}`,
        guidance,
      });
    } else {
      runtime.error([`PASO requires working inference: ${inference.error}`, guidance].join("\n"));
    }
    if (!requestExitAfterOneShotOutput(runtime, 1)) {
      runtime.exit(1);
    }
  ...
  runtime.log("PASO requires working inference. Starting guided AI setup…");
