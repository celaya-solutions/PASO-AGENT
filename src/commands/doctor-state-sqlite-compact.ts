...
export async function runDoctorStateSqliteCompact(
  ...
  if (!stat.isFile()) {
    throw new Error(`Canonical PASO state database is not a regular file: ${sqlitePath}`);
  }
      ...
      runWithOpenClawStateWriteAccess(
        ...
        () => {
          if (isOpenClawStateDatabaseOpen()) {
            throw new Error(
              "The shared PASO state database is already open in this process. Stop PASO and retry.",
            );
          }
          ...
          const compact = compactDoctorSqliteFile({
            afterSuccess: () => {
              if (!clearOpenClawDatabaseQuarantine(sqlitePath, { env })) {
                throw new Error(
                  `PASO state database ${sqlitePath} was compacted, but its persisted quarantine record could not be cleared. Rerun openclaw doctor --fix so the database is not refused again.`,
                );
              }
