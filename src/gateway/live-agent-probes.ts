...
export function buildLiveCronProbeMessage(params: {
  ...
  if (params.attempt === 0) {
    return (
      "Use the PASO MCP automations tool from server `openclaw`. " +
      "If it is not already visible, search/load MCP tools for `openclaw automations` or `automations`, " +
      "then call the matching PASO MCP tool; Claude-style names may appear as `mcp__openclaw__automations`. " +
      "Do not use Claude native `CronCreate`, `CronList`, or `CronDelete`; those are not PASO proof. " +
      ...
      "Do the actual tool call; I will verify externally with the PASO cron CLI. " +
      `After the cron job is created, reply exactly: ${params.exactReply}`
    );
  }
  if (claudeLike) {
    return (
      "Retry the PASO MCP automations tool from server `openclaw` now. " +
      "If it is not already visible, search/load MCP tools for `openclaw automations` or `automations`, " +
      "then call the matching PASO MCP tool; Claude-style names may appear as `mcp__openclaw__automations`. " +
      "Do not use Claude native `CronCreate`, `CronList`, or `CronDelete`; those are not PASO proof. " +
      ...
      "I will verify externally with the PASO cron CLI."
    );
  }
  return (
    "Your previous PASO automations MCP tool call was cancelled before the job was created. " +
    "Retry the PASO MCP automations tool from server `openclaw` now. " +
