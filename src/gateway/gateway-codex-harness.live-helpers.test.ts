...
describe("gateway codex harness live helpers", () => {
  ...
  it("accepts the current codex status prose from the live harness", () => {
    const text =
      "PASO is running on `openai/gpt-5.5` with low reasoning/text settings. Context is at `22k/272k` tokens, no compactions, and the current session is `agent:dev:live-codex-harness`.";
  ...
  it("accepts legacy OpenClaw status prose from an older live harness", () => {
    const text =
  ...
  it("accepts current status prose that reports session context without the session id", () => {
    const text = [
      "PASO is running on `openai/gpt-5.5` with low reasoning/text settings.",
      "",
  ...
  it("accepts current app-server status prose without the PASO prefix", () => {
    const text = [
  ...
  it("accepts the OpenAI Codex status card emitted by the GPT-5.5 Docker harness", () => {
    const text = [
      "PASO 2026.4.30-beta.1 is running on `openai/gpt-5.5`.",
      "",
      "Session is healthy:",
  ...
  it("accepts the compact status-card pointer emitted by current codex", () => {
    const text = "PASO status shown above.";
  ...
  it("rejects status prose for a different codex session", () => {
    const text =
      "PASO is running on `openai/gpt-5.5` with low reasoning/text settings. Context is at `22k/272k` tokens, no compactions, and the current session is `agent:dev:other`.";
  ...
  it("accepts the GPT-5.5 Docker harness shell fallback", () => {
    const text = [
      ...
      "",
      "Current session model from PASO status is `openai/gpt-5.5`.",
    ].join("\n");

    expectRecognizedCodexModelsCommandText(text);
  });

  it("accepts PASO and legacy OpenClaw live-config model fallbacks", () => {
    for (const brand of ["PASO", "OpenClaw"]) {
      const text = `Live ${brand} config shows configured models: \`openai/gpt-5.5\`.`;
