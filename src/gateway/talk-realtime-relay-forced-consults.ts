...
function buildForcedConsultCheckingPrompt(): string {
  return [
    "Briefly tell the person that you are checking with PASO.",
    "Do not answer the request yet. Wait for the PASO result before giving the actual answer.",
  ].join(" ");
}

function buildForcedConsultSpeechPrompt(text: string): string {
  return [
    "PASO finished checking. Speak this result naturally and concisely.",
...
export function buildAlreadyDeliveredToolResult(): Record<string, string> {
  return {
    ...
    message: "PASO already delivered this consult result internally. Do not repeat it.",
  };
}
  ...
  session.harness.forcedConsults.schedule(handle, FORCED_CONSULT_FALLBACK_DELAY_MS, () => {
    ...
    broadcastToOwner(session.context, session.connId, {
      ...
      args: {
        ...
        context:
          "The realtime provider produced a final user transcript without invoking openclaw_agent_consult, so PASO is forcing the consult for realtime Talk.",
...
export function submitForcedTalkRealtimeRelayToolResult(
  ...
  if (cancelled) {
    const providerResult = buildRealtimeVoiceAgentCancelProviderResult(
      "PASO cancelled this consult before completion. Do not restart it.",
    );
