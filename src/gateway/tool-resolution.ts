  ...
  const allTools = nodeExecDefaults
    ? [
        ...
        createLazyExecTool(
          ...
          {
            description:
              "Execute a shell command on a connected PASO node. This tool is node-only; use the CLI native shell for Gateway-local commands. Commands run synchronously. Set node when multiple nodes are available.",
