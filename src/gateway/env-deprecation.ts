...
// Emits a one-shot notice for ignored pre-PASO environment prefixes.
...
export function warnLegacyOpenClawEnvVars(env: NodeJS.ProcessEnv = process.env): void {
  ...
  process.emitWarning(
    [
      `Legacy ${detectedPrefixes} environment variables were detected (${legacyVarCount} total), but PASO only reads OPENCLAW_* names now.`,
