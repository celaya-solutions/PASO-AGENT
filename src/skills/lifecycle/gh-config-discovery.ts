...
// PASO process is running with a different HOME (e.g. the per-agent
...
type GhConfigDirMismatch = {
  ...
  // Suggested env value the operator should set on the PASO service to
...
export function formatGhConfigDirMismatchHint(mismatch: GhConfigDirMismatch): string[] {
  const lines: string[] = [
    "GitHub CLI auth was found at a different HOME than the one this PASO process uses.",
  ...
  lines.push(
    `  Fix: set GH_CONFIG_DIR=${mismatch.suggestedEnvValue} on the PASO service environment, then restart the gateway.`,
  );
  return lines;
}
