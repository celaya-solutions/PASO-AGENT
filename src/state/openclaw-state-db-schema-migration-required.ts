...
export class OpenClawStateDatabaseSchemaMigrationRequiredError extends Error {
  ...
  constructor(
    ...
    super(
      `PASO state database schema migration required (${kind}) at ${pathname}; run openclaw doctor --fix to migrate it.`,
    );
...
const STATE_SCHEMA_MIGRATION_REQUIRED_MESSAGE =
  /^(?:PASO|OpenClaw) state database schema migration required \((agent-databases-composite-primary-key|audit-events-v2)\) at (.+); run openclaw doctor --fix to migrate it\.$/u;
