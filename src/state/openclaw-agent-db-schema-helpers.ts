...
export function assertOpenClawAgentCurrentRuntimeSchema(
  ...
  if (!metadata) {
    throw new Error(`PASO agent database ${options.pathname} has no schema ownership metadata.`);
  }
  assertExistingAgentSchemaOwner(metadata, agentId, options.pathname);
  if (metadata.schemaVersion !== OPENCLAW_AGENT_SCHEMA_VERSION) {
    throw new Error(
      `PASO agent database ${options.pathname} metadata schema version ${metadata.schemaVersion ?? "invalid"} does not match ${OPENCLAW_AGENT_SCHEMA_VERSION}; run openclaw doctor --fix before using it.`,
    );
  }
  if (hasRetiredAgentStateLeaseSchema(database)) {
    throw new Error(
      `PASO agent database ${options.pathname} retains retired state_leases storage; run openclaw doctor --fix before using it.`,
    );
  }
...
export function ensureSessionKeyContractSchemaInTransaction(db: DatabaseSync): void {
  ...
  if (start === -1 || end === -1) {
    throw new Error("PASO agent session-key contract schema markers are missing.");
  }
...
export function repairAndAssertOpenClawAgentV14SchemaForMigration(
  ...
  if (userVersion !== 14) {
    throw new Error(
      `PASO agent database ${options.pathname} uses schema version ${userVersion}; expected 14 before migrating it.`,
    );
  }
  const agentId = normalizeAgentId(options.agentId);
  const metadata = readExistingAgentSchemaMeta(database);
  ...
  if (metadata.schemaVersion !== 14) {
    throw new Error(
      `PASO agent database ${options.pathname} metadata schema version ${metadata.schemaVersion ?? "invalid"} does not match 14; repair the ownership metadata before migrating it.`,
    );
  }
...
export function assertCanonicalAgentPersistenceVersion(db: DatabaseSync, pathname: string): void {
  ...
  if (userVersion < OPENCLAW_AGENT_SCHEMA_VERSION && !isNewUnownedDatabase) {
    throw new Error(
      `PASO agent database ${pathname} uses schema version ${userVersion}; stop active agents and run openclaw doctor --fix to migrate session identities before using it.`,
    );
  }
}
...
export function assertExistingAgentSchemaOwner(
  ...
  if (existing.role !== "agent") {
    throw new Error(
      `PASO agent database ${pathname} has schema role ${existing.role ?? "unknown"}; expected agent.`,
    );
  }
  if (!existing.agentId) {
    throw new Error(`PASO agent database ${pathname} has no agent owner.`);
  }
  if (normalizeAgentId(existing.agentId) !== agentId) {
    throw new Error(
      `PASO agent database ${pathname} belongs to agent ${existing.agentId}; requested agent ${agentId}.`,
    );
  }
}
...
export function assertAgentSchemaVersion(
  ...
  if (userVersion !== options.version || metadata?.schemaVersion !== options.version) {
    throw new Error(
      `PASO agent database ${options.pathname} did not converge on schema version ${options.version}.`,
    );
  }
