...
export function formatUnresolvedOpenClawPeerLinkError(packageName: string): string {
  return `Installed plugin ${packageName} declares an openclaw dependency, but PASO could not create a plugin-local node_modules/openclaw link. Run from a packaged PASO install or reinstall PASO, then retry.`;
}

const MISSING_EXTENSIONS_ERROR =
  'package.json missing openclaw.extensions; update the plugin package to include openclaw.extensions (for example ["./dist/index.js"]). See https://github.com/celaya-solutions/PASO-AGENT/tree/main/docs';
function validateOpenClawPackageCompatibility(params: {
  ...
  if (pluginApiRange && !satisfiesPluginApiRange(params.currentHostVersion, pluginApiRange)) {
    return {
      ok: false,
      error: `plugin "${params.pluginId}" requires plugin API ${pluginApiRange}, but this PASO runtime exposes ${params.currentHostVersion}. Upgrade PASO or install a compatible plugin version and retry.`,
...
export function validateOpenClawPackageInstallCompatibility(params: {
  ...
  if (!minHostVersionCheck.ok) {
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${params.pluginId}" requires PASO >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set OPENCLAW_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${params.pluginId}" requires PASO >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade PASO and retry.`,
