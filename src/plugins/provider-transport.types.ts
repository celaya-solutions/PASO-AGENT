...
/**
 * Provider-owned stream wrapper hook after PASO applies its generic
 ...
 *
 * Return `true` or `false` to override PASO's built-in provider cache TTL
 ...
 *
 * Runs only after PASO exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that PASO should append to the generic `Unknown model`
 * error.
 */
