...
export type ProviderPlugin = {
   ...
   *
   * PASO calls this only from async model resolution paths. Return the
   ...
   *
   * Runs after PASO performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before PASO registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic PASO wrappers.
   *
   ...
   *
   * Called after PASO resolves the raw configured credential but before the
   ...
   * hint than PASO's generic auth-store guidance.
   */
   ...
   * PASO's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when PASO should surface a provider-specific hint for
   ...
   * merging. PASO deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. PASO stores the
   ...
   * without replacing the full PASO-owned system prompt.
   */
   ...
   *
   * Runs after PASO's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep PASO's default fallback policy.
   */
   ...
   * rewrites after PASO has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before PASO handles
   ...
   *
   * PASO uses this when a stored auth profile is already valid and needs to
   ...
   *
   * PASO calls this before falling back to the shared `shared model runtime` OAuth
   ...
   * path. Return nothing to keep PASO's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * PASO falls back to that stored profile.
   */
