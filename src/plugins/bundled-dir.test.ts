...
describe("resolveBundledPluginsDir", () => {
  ...
  it("reports missing pnpm workspace deps for source checkouts", () => {
    ...
    expect(resolveSourceCheckoutDependencyDiagnostic()).toEqual({
      source: repoRoot,
      message:
        "PASO source checkout detected without pnpm workspace dependencies; run `pnpm install` from the repo root so bundled plugins can load package-local dependencies.",
    });
