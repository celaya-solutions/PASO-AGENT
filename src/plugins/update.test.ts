...
describe("updateNpmInstalledPlugins", () => {
  ...
  it("disables a corrupt installed payload when metadata probing also fails", async () => {
    ...
    const message =
      'Disabled "lossless-claw" after plugin update failure; PASO will continue without it. Failed to check lossless-claw: npm view failed: registry timeout';
  ...
  it("disables a missing plugin payload when metadata probing also fails", async () => {
    ...
    const message =
      'Disabled "demo" after plugin update failure; PASO will continue without it. Failed to update demo: npm view failed: registry timeout';
    expect(warn).toHaveBeenCalledWith(message);
    expect(result.changed).toBe(true);
  ...
  it("disables a blocked ClawHub plugin without changing trust policy", async () => {
    ...
    expect(result.config.plugins?.slots).toBeUndefined();
    const message =
      'Disabled "demo" after plugin update failure; PASO will continue without it. Failed to update demo: ClawHub blocked this release; update was not started. (ClawHub clawhub:demo).';
    expect(warn).toHaveBeenCalledWith(message);
    expect(result.outcomes).toEqual([
      {
        pluginId: "demo",
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("does not fall back from ClawHub to non-PASO npm packages", async () => {
    resolveBundledPluginSourcesMock.mockReturnValue(new Map());
    installPluginFromClawHubMock.mockResolvedValue({
      ok: false,
      code: "package_not_found",
