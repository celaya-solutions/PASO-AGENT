...
describe("loadWebMedia", () => {
  ...
  it("allows trusted generated host-read HTML reports under PASO temp root", async () => {
  ...
  it("rejects host-read HTML files outside the trusted PASO temp root", async () => {
  ...
  it("rejects trusted host-read HTML symlinks that resolve outside PASO temp root", async () => {
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), "web-media-host-html-"));
  ...
  it("rejects trusted host-read HTML hardlinks to files outside PASO temp root", async () => {
