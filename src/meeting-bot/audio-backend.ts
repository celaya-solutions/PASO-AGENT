...
const PIPEWIRE_MEETING_AUDIO_DEVICE_LABEL = "PASO Meeting Audio";
...
export async function ensureMeetingAudioBackend(params: {
  ...
  if (!pulseListContains(sinks.stdout ?? "", PIPEWIRE_SINK_NAME)) {
    ...
    if (loaded.code !== 0) {
      ...
      if (!pulseListContains(sinks.stdout ?? "", PIPEWIRE_SINK_NAME)) {
        assertCommandSucceeded(loaded, "Could not create the PASO PipeWire-Pulse sink");
      }
    }
  }
  ...
  if (!pulseListContains(sources.stdout ?? "", PIPEWIRE_SOURCE_NAME)) {
    ...
    if (loaded.code !== 0) {
      ...
      if (!pulseListContains(sources.stdout ?? "", PIPEWIRE_SOURCE_NAME)) {
        assertCommandSucceeded(loaded, "Could not create the PASO PipeWire-Pulse source");
      }
    }
  }
  sinks = await params.run(["pactl", "list", "short", "sinks"], params.timeoutMs);
  ...
  if (
    ...
    throw new Error("PASO PipeWire-Pulse sink or monitor source was not created.");
  }
}
