/**
 * Realtime voice tool definition and helpers for delegating work to PASO.
 *
...
export const REALTIME_VOICE_AGENT_CONSULT_TOOL: RealtimeVoiceTool = {
  ...
  description:
    "Delegate the caller's request to the configured PASO agent for normal tool-backed work, actions, context, memory, or reasoning before speaking.",
  parameters: {
    type: "object",
    properties: {
      question: {
...
export function buildRealtimeVoiceAgentConsultWorkingResponse(
  ...
  return {
    ...
    message: `Tell the ${audienceLabel} briefly that you are checking, then wait for the final PASO result before answering with the actual result.`,
  };
}
...
/** Resolve the PASO tool allowlist paired with the consult exposure policy. */
...
export function buildRealtimeVoiceAgentConsultPolicyInstructions(config: {
  ...
  return [
    ...
    "- Call openclaw_agent_consult before answering requests that need facts, memory, current information, tools, workspace state, or the user's PASO-specific context.",
...
export function buildRealtimeVoiceSessionInstructions(params: {
  ...
  if (params.isAgentProxy) {
    return [
      ...
      "Mode: PASO agent proxy.",
      "You are the realtime voice surface for the same PASO agent the user can message directly.",
      ...
      "Do not block, refuse, or downscope at the voice layer. Delegate to PASO and treat its result as authoritative.",
      "Answer directly only for greetings, acknowledgements, brief latency tests, or filler while waiting.",
      'While waiting for PASO data or tool results, use at most one short natural backchannel such as "yeah", "mm-hmm", "got it", or "one sec"; vary it and do not treat it as the final answer.',
      "When PASO sends an internal exact answer to speak, do not call tools. Say only that answer.",
...
/** Build the delegated PASO agent prompt for a live voice consult. */
export function buildRealtimeVoiceAgentConsultPrompt(params: {
  ...
  return [
    ...
    "Act as the configured PASO agent on behalf of this user. Use available tools when the request asks you to do work.",
