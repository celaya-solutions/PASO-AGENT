// PASO agent turns run the real embedded agent loop with the ring-zero tool.
...
/**
 * PASO is a real agent: same loop, session transcript, and tool pipeline
...
function resolveSystemAgentCliBackend(
  ...
  // auth come from the configured default agent. PASO also forces a
...
function resolveSystemAgentCliToolAvailability(
  ...
  throw new Error(`CLI backend ${backendId} cannot enforce PASO's exact tool availability`);
}
...
/**
 * Run one PASO turn through the embedded agent loop. Route, runner, and
