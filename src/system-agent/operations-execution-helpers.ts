...
import { resolvePasoDocsUrl } from "../../packages/terminal-core/src/links.js";
...
export function formatChannelDocsUrl(docsPath: string): string {
  return resolvePasoDocsUrl(docsPath);
}
...
export async function applyPersistentOperation(params: {
  ...
  try {
    ...
    runtime.error(
      `${outcome.summary}, but PASO could not record its audit entry: ${formatErrorMessage(error)}`,
    );
  }
...
export async function assertConfigWriteDoesNotBypassInferenceVerification(
  ...
  if (verdict === "plugin-entry") {
    ...
    throw new Error(
      `Direct config writes cannot change plugin "${pluginId}" because it may back PASO's own active inference route. Editing it is a human-only change, made with PASO stopped from a trusted shell on the machine running it.`,
    );
  }
  ...
  throw new Error(
    denialReason
      ...
      : "Direct config writes cannot change the default inference route or include alternate config. Use `set_default_model` (optionally with agentId) for an already configured route; changing provider or auth access is `openclaw onboard` on the machine running PASO.",
  );
}
...
async function verifyCurrentSetupInference(
  ...
  if (!before.exists || !before.valid) {
    throw new Error(
      "PASO setup requires a valid configured inference route. Run `openclaw onboard` on the machine running PASO, then retry.",
    );
  }
  ...
  if (!beforeRoute.route) {
    throw new Error(
      "PASO setup requires working inference first. Run `openclaw onboard` on the machine running PASO, then retry.",
    );
  }
  ...
  if (!verification.ok) {
    throw new Error(
      `PASO setup requires working inference first. The configured route failed a live check: ${verification.error} Run \`openclaw onboard\` on the machine running PASO, then retry.`,
    );
  }
...
export async function executeSetup(
  ...
  if (!defaultModel) {
    throw new Error(
      "PASO setup requires working inference first. Run `openclaw onboard` on the machine running PASO to configure and verify a default model, then start PASO again.",
    );
  }
  const requestedModel = operation.model?.trim();
  if (requestedModel && requestedModel !== defaultModel) {
    throw new Error(
      `PASO setup will preserve the verified default model ${defaultModel}. Staging, live-testing, and saving a different inference route is \`openclaw onboard\` on the machine running PASO.`,
    );
  }
  if (!opts.approved) {
    const message = [
  ...
  if (requestedModel && requestedModel !== verified.modelRef) {
    throw new Error(
      `The verified default model is now ${verified.modelRef}, not ${requestedModel}. Review the current route, or run \`openclaw onboard\` on the machine running PASO, before retrying setup.`,
    );
  }
      ...
      const result = await mutateConfigFile({
        ...
        writeOptions: {
          ...
          preCommitRuntimePreflight: async (sourceConfig) => {
            ...
            // The live probe can outlive the original PASO authority.
