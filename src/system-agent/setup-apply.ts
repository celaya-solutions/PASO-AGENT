// Applies PASO's conversational setup: config, workspace files, gateway.
...
export async function applySystemAgentSetup(
  ...
  if (hasExpectedConfigHash && resolveConfigSnapshotHash(snapshot) !== expectedConfigHash) {
    throw new Error("PASO config changed while AI access was being tested. Try setup again.");
  }
  ...
  const assertVerifiedRoute = async (
    ...
    if (
      ...
      throw new Error(
        phase === "before"
          ? "The default-agent inference route changed before setup could start, so no workspace or Gateway settings were changed. Retry setup from the current PASO session."
          : "The default-agent inference route changed after the config write, so no further setup effects were applied. Retry setup from the current PASO session.",
      );
    }
    return currentRoute;
  };
  ...
  if (startedWithoutAuthoredRoster) {
    ...
    if (!created.createdAgent || !created.configHash) {
      throw new Error(
        "PASO did not create the approved first agent because the roster changed. Retry setup.",
      );
    }
    ...
    if (
      ...
      throw new Error("PASO first-agent ownership changed during setup. Retry setup.");
    }
      ...
      await transformConfigWithPendingPluginInstalls({
        ...
        transform: async (currentConfig, context) => {
          ...
          if (
            ...
            throw new Error(
              "PASO config changed while AI access was being tested. Try setup again.",
            );
          }
          ...
          if (
            ...
            throw new Error(
              "The setup candidate no longer preserves the exact verified inference route, so it was not saved. Retry setup from the current PASO session.",
            );
          }
  ...
  if (!settings) {
    throw new Error("PASO setup committed without resolved Gateway settings.");
  }
  ...
  if (verifiedRoute) {
    ...
    if (!expectedRuntime.ok) {
      ...
      throw new Error(
        `PASO could not validate the setup route after its config write${detail}. No further setup effects were applied. Retry setup from the current PASO session.`,
      );
    }
    ...
    if (!sameSetupConfiguredRoute(expectedPersistedRoute.route, verifiedRoute.route, false)) {
      throw new Error(
        "The materialized inference route no longer matches the exact verified route, so no further setup effects were applied. Retry setup from the current PASO session.",
      );
    }
  }
  ...
  // Setup approval includes consent for PASO's local model harnesses.
  ...
  await runCommittedFollowUp(
    ...
    (error) =>
      lines.push(
        `PASO exec approval: ${formatErrorMessage(error)}; local model harnesses may ask again.`,
      ),
  );
