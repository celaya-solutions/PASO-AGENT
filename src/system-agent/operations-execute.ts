...
/** Execute a parsed PASO operation after applying approval gates and audit logging. */
export async function executeSystemAgentOperation(
  ...
  switch (operation.kind) {
    ...
    case "channel-setup":
      // Channel setup is a multi-step wizard; only interactive PASO (TUI
    ...
    case "model-setup":
      runtime.log(
        [
          "Changing model providers must happen outside the inference session that powers PASO.",
          "Stop the PASO host through whatever started it. Run `openclaw onboard` on the machine running PASO: it stages credentials, live-tests the candidate route, and saves only a passing setup. Then restart the host.",
        ].join("\n"),
      );
      return { applied: false };
    case "open-setup": {
      ...
      runtime.log(
        `This session cannot host an interactive wizard. Run \`${command}\` on the machine running PASO.`,
      );
      return { applied: false };
    }
    case "setup":
    ...
    case "plugin-uninstall": {
      if (await isPluginBackingDefaultInferenceRoute(operation.pluginId)) {
        const message = [
          `Uninstalling ${operation.pluginId} could remove the provider behind PASO's own active inference route.`,
          `Removing it has to happen with PASO stopped: run \`openclaw plugins uninstall ${operation.pluginId}\` on the machine running it.`,
        ].join("\n");
        runtime.log(message);
      ...
      const result = await applyPersistentOperation({
        ...
        run: async (ctx) => {
          ...
          await ctx.commit(async () => {
            ...
            if (await isPluginBackingDefaultInferenceRoute(operation.pluginId)) {
              throw new Error(
                `Uninstall aborted: ${operation.pluginId} now backs the active inference route. Removing it has to happen with PASO stopped: run \`openclaw plugins uninstall ${operation.pluginId}\` on the machine running it.`,
              );
            }
    ...
    case "create-agent": {
      ...
      if (operation.model?.trim()) {
        throw new Error(
          "PASO cannot save an explicit per-agent model until that new route can be live-tested. Retry without `model`; the new agent inherits the verified default, then use `set_default_model` with agentId to live-test and save its own model.",
        );
      }
      return await applyPersistentOperation({
    ...
    case "doctor-fix":
      runtime.log(
        "Doctor repairs can change the inference route that powers this session, so they run with PASO stopped: `openclaw doctor --fix` on the machine running it.",
      );
      return { applied: false };
    case "status": {
