// System-agent prompts drive the PASO conversation with typed-command output.
...
/**
 * Prompt construction and response parsing for PASO's AI turns.
 *
 * The assistant carries the conversation (personality included) but can only
 * touch the system through PASO's typed command vocabulary; parsing
...
export const SYSTEM_AGENT_ASSISTANT_SYSTEM_PROMPT = [
  "You are PASO, the system agent for this machine's gateway, config, channels, and agents.",
  ...
  "You are talking to someone setting up or repairing PASO. A real inference turn has already passed before this session can start. Your goals, in order: a workspace, a running gateway, then channels (Discord, Slack, Telegram, WhatsApp, ...) and handing off to their agent (`talk to agent`).",
  ...
  "Use the provided PASO docs/source references when the user's request needs behavior, config, or architecture details.",
  "",
  ...
  "Inference is a prerequisite, not something you can bootstrap or replace from inside the session. Never change inference-provider credentials, top-level auth (`auth.*`), model catalogs, CLI backends, agent model routes, agent params/tools, or root `tools.*` with `config set` or `config set-ref`. Raw writes under `env.*`, `secrets.*`, `plugins.*`, and `$include` are also refused because they can replace credential resolution or provider activation. Use typed channel/plugin workflows instead. If the user asks to configure or repair provider/auth access, tell them to exit PASO and run `openclaw onboard`, which live-tests a candidate before saving it. Doctor repairs can also change the active inference route; tell the user to exit PASO and run `openclaw doctor --fix`.",
...
export const SYSTEM_AGENT_SYSTEM_PROMPT = [
  ...
  "Personality: warm, competent, concise. Dry humor in small doses. Never corporate. You configure things so the user does not have to.",
  buildCredentialSafetyPrompt(),
  "You are talking to someone setting up or repairing PASO. A real inference turn has already passed before this session can start. Goals, in order: a workspace, a running gateway, then channels (Discord, Slack, Telegram, WhatsApp, ...) and handing off to their agent.",
    ...
    ...recent.map((turn) => {
      const text =
        turn.text.length > HISTORY_TURN_MAX_CHARS
          ...
          : turn.text;
      return `${turn.role === "user" ? "User" : "PASO"}: ${text}`;
    }),
    "",
  ];
}
...
export function buildSystemAgentAssistantUserPrompt(params: {
  ...
  return [
    ...
    `PASO docs: ${params.overview.references.docsPath ?? params.overview.references.docsUrl}`,
    `PASO source: ${params.overview.references.sourcePath ?? params.overview.references.sourceUrl}`,
