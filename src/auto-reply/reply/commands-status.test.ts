...
describe("buildStatusReply subagent summary", () => {
  ...
  it("does not leak internal runtime context through the task status line", async () => {
    ...
    expect(reply?.text).not.toContain("PASO runtime context (internal):");
  ...
  it("shows the effective non-PASO embedded harness in /status", async () => {
  ...
  it("uses Codex OAuth auth labels for explicit OpenAI PASO auth order", async () => {
    await withTempHome(
      async (dir) => {
        const agentDir = path.join(dir, ".openclaw", "agents", "main", "agent");
        fs.mkdirSync(agentDir, { recursive: true });
  ...
  it("keeps /status on an explicit PASO runtime override after config changes", async () => {
    registerStatusCodexHarness();
